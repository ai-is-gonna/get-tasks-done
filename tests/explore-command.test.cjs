...
describe('explore command', () => {
  test('command file exists', () => {
    const p = path.join(__dirname, '..', 'commands', 'gtd', 'explore.md');
    assert.ok(fs.existsSync(p), 'commands/gtd/explore.md should exist');
  });

  test('command file has required frontmatter', () => {
    const p = path.join(__dirname, '..', 'commands', 'gtd', 'explore.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('name: gtd:explore'), 'Command must have name frontmatter');
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'get-tasks-done', 'workflows', 'explore.md');
