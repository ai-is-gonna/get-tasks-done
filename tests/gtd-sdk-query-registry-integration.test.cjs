/**
 * Drift guard: every `gtd-sdk query <cmd>` reference in the repo must
...
const COMMAND_ALIASES_FILE = path.join(REPO_ROOT, 'get-tasks-done', 'bin', 'lib', 'command-aliases.generated.cjs');

// Prose tokens that repeatedly appear after `gtd-sdk query` in English
...
function collectReferences() {
  ...
  const refs = [];
  const re = /gtd-sdk\s+query\s+([A-Za-z][-A-Za-z0-9._/]+)(?:\s+([A-Za-z][-A-Za-z0-9._]+))?/g;
...
describe('gtd-sdk query registry integration', () => {
  test('every referenced command resolves to a registered handler', () => {
    ...
    assert.ok(refs.length > 0, 'expected to find gtd-sdk query references');
    ...
    assert.strictEqual(
      offenders.length, 0,
      'Referenced `gtd-sdk query <cmd>` tokens with no handler in ' +
  ...
  test('informational: handlers with no in-repo caller', () => {
    ...
    if (unused.length > 0 && process.env.GTD_LOG_UNUSED_HANDLERS) {
