...
describe('SPAWN: spawn type consistency', () => {
  test('no "First, read agent .md" workaround pattern remains', () => {
    ...
    for (const dir of dirs) {
      ...
      for (const file of files) {
        ...
        const hasWorkaround = content.includes('First, read ~/.claude/agents/gtd-');
        assert.ok(
          !hasWorkaround,
  ...
  test('diagnose-issues uses gtd-debugger (not general-purpose)', () => {
    const content = fs.readFileSync(
      path.join(WORKFLOWS_DIR, 'diagnose-issues.md'), 'utf-8'
    );
    assert.ok(
      content.includes('subagent_type="gtd-debugger"'),
      'diagnose-issues should spawn gtd-debugger, not general-purpose'
    );
  });
  ...
  test('workflows spawning named agents have <available_agent_types> listing (#1357)', () => {
    ...
    // PR #1139 added this to plan-phase and work-task-issue but missed all other
    // workflows that spawn named GTD agents.
    const dirs = [WORKFLOWS_DIR, COMMANDS_DIR];
    for (const dir of dirs) {
      if (!fs.existsSync(dir)) continue;
      const files = fs.readdirSync(dir).filter(f => f.endsWith('.md'));
  ...
  test('work-task-issue exposes task executor and phase finalization contracts', () => {
    const content = fs.readFileSync(
      path.join(WORKFLOWS_DIR, 'work-task-issue.md'), 'utf-8'
    );
    assert.ok(
      content.includes('gtd-task-executor'),
      'work-task-issue must delegate implementation to gtd-task-executor'
    );
    assert.ok(
      content.includes('<phase_completion_mode>'),
      'work-task-issue must document the complete-phase finalization mode'
    );
  });
});
...
describe('CLAUDEMD: CLAUDE.md compliance enforcement', () => {
  test('gtd-plan-checker has Dimension 10: CLAUDE.md Compliance', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-plan-checker.md'), 'utf-8');
    assert.ok(
      content.includes('Dimension 10: CLAUDE.md Compliance'),
      'gtd-plan-checker must have Dimension 10 for CLAUDE.md compliance checking'
    );
    assert.ok(
      content.includes('claude_md_compliance'),
      'gtd-plan-checker must use claude_md_compliance as dimension identifier'
    );
  });

  test('gtd-phase-researcher has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'gtd-phase-researcher must enforce CLAUDE.md directives during research'
    );
    assert.ok(
      content.includes('Project Constraints (from CLAUDE.md)'),
      'gtd-phase-researcher must output a Project Constraints section from CLAUDE.md'
    );
  });

  test('gtd-task-executor has CLAUDE.md enforcement directive', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-task-executor.md'), 'utf-8');
    assert.ok(
      content.includes('CLAUDE.md enforcement'),
      'gtd-task-executor must enforce CLAUDE.md directives during execution'
    );
    assert.ok(
      content.includes('CLAUDE.md rule — it takes precedence over plan instructions'),
      'gtd-task-executor must specify CLAUDE.md precedence over plan instructions'
    );
  });

  test('all three agents read CLAUDE.md in project_context', () => {
    const agents = ['gtd-plan-checker', 'gtd-phase-researcher', 'gtd-task-executor'];
...
describe('VERIFY: data-flow trace, environment audit, and behavioral spot-checks', () => {
  test('gtd-verifier has Step 4b: Data-Flow Trace', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 4b: Data-Flow Trace'),
      'gtd-verifier must have Step 4b for data-flow tracing'
    );
    assert.ok(
      content.includes('HOLLOW'),
      'gtd-verifier must define HOLLOW status for wired-but-disconnected artifacts'
    );
    assert.ok(
      content.includes('DISCONNECTED'),
      'gtd-verifier must define DISCONNECTED status for missing data sources'
    );
  });

  test('gtd-verifier has Step 7b: Behavioral Spot-Checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Step 7b: Behavioral Spot-Checks'),
      'gtd-verifier must have Step 7b for behavioral spot-checks'
    );
    assert.ok(
      content.includes('SKIP'),
      'gtd-verifier spot-checks must support SKIP status for untestable items'
    );
  });

  test('gtd-verifier VERIFICATION.md template includes data-flow and spot-check sections', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-verifier.md'), 'utf-8');
    assert.ok(
  ...
  test('gtd-verifier success criteria include data-flow and spot-checks', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-verifier.md'), 'utf-8');
    assert.ok(
      content.includes('Data-flow trace (Level 4)'),
  ...
  test('gtd-phase-researcher has Step 2.6: Environment Availability Audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-phase-researcher.md'), 'utf-8');
    assert.ok(
      content.includes('Step 2.6: Environment Availability Audit'),
      'gtd-phase-researcher must have Step 2.6 for environment availability auditing'
    );
    assert.ok(
      content.includes('Environment Availability'),
      'gtd-phase-researcher must include Environment Availability section in RESEARCH.md template'
    );
  });

  test('gtd-phase-researcher success criteria include environment audit', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gtd-phase-researcher.md'), 'utf-8');
    assert.ok(
...
describe('DISCUSS: discussion log generation', () => {
  ...
  test('discussion-log template exists', () => {
    const templatePath = path.join(__dirname, '..', 'get-tasks-done', 'templates', 'discussion-log.md');
    assert.ok(
...
describe('COMPAT: agents must not use runtime-specific frontmatter keys', () => {
  ...
  const AGENTS_WITH_WRITE = ['gtd-task-executor', 'gtd-debugger'];
