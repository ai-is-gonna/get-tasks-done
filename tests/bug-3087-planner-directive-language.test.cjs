...
//
// Between v1.38.3 and v1.38.4, agents/gtd-planner.md had 10 instances of
...
let src;
try {
  src = fs.readFileSync(path.join(ROOT, 'agents', 'gtd-planner.md'), 'utf8');
} catch (err) {
  throw new Error(`agents/gtd-planner.md not found — was the file renamed? (${err.message})`);
}
...
for (const { desc, pattern } of directives) {
  test(`gtd-planner.md: ${desc}`, () => {
    assert.ok(
      pattern.test(src),
      `Directive enforcement missing from gtd-planner.md: "${desc}" — pattern ${pattern} not found. ` +
