...
describe('AUDIT-FIX: command file', () => {
  ...
  test('command file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in commands/gtd/'
    );
  });

  test('has valid frontmatter with name gtd:audit-fix', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('name: gtd:audit-fix'),
      'name must be gtd:audit-fix'
    );
  });
  ...
  test('has argument-hint in frontmatter for /gtd-help discoverability', () => {
    const content = fs.readFileSync(cmdPath, 'utf-8');
    const frontmatter = content.split('---')[1] || '';
    assert.ok(
      frontmatter.includes('argument-hint:'),
      'must have argument-hint in frontmatter for /gtd-help discoverability'
    );
  });
...
describe('AUDIT-FIX: workflow file', () => {
  ...
  test('workflow file exists', () => {
    assert.ok(
      ...
      'audit-fix.md must exist in get-tasks-done/workflows/'
    );
  });
  ...
  test('has <available_agent_types> listing gtd-task-executor', () => {
    const content = fs.readFileSync(wfPath, 'utf-8');
    assert.ok(
    ...
    assert.ok(
      ...
      'must list gtd-task-executor as available agent type'
    );
  });
});
