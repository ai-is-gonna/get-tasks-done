...
describe('init commands', () => {
  ...
  test('init plan-phase returns file paths', () => {
    ...
    const result = runGtdTools('init plan-phase 03', tmpDir);
  ...
  test('init plan-phase respects model_overrides for planner_model', () => {
    const phaseDir = path.join(tmpDir, '.planning', 'phases', '01-foundation');
    fs.mkdirSync(phaseDir, { recursive: true });
    fs.writeFileSync(path.join(phaseDir, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(tmpDir, '.planning', 'config.json'), JSON.stringify({
      model_profile: 'balanced',
      model_overrides: { 'gtd-planner': 'openai/o4-mini' },
    }));

    const result = runGtdTools('init plan-phase 1 --raw', tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.planner_model, 'openai/o4-mini',
      'model_overrides["gtd-planner"] must take precedence over profile');
  });

  test('init plan-phase respects planner model_overrides when resolve_model_ids is omit', () => {
    const phaseDir = path.join(tmpDir, '.planning', 'phases', '01-foundation');
    fs.mkdirSync(phaseDir, { recursive: true });
    fs.writeFileSync(path.join(phaseDir, '01-01-PLAN.md'), '# Plan');
    fs.writeFileSync(path.join(tmpDir, '.planning', 'config.json'), JSON.stringify({
  ...
  test('init progress returns file paths', () => {
    const result = runGtdTools('init progress', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init phase-op returns core and optional phase file paths', () => {
    ...
    const result = runGtdTools('init phase-op 03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init plan-phase extracts phase_req_ids from ROADMAP', () => {
    ...
    const result = runGtdTools('init plan-phase 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
// ─────────────────────────────────────────────────────────────────────────────
// ROADMAP fallback for init plan-phase / task orchestration / verify-work (#1238)
// ─────────────────────────────────────────────────────────────────────────────
...
describe('init commands ROADMAP fallback when phase directory does not exist (#1238)', () => {
  ...
  test('init verify-work falls back to ROADMAP when no phase directory exists', () => {
    const result = runGtdTools('init verify-work 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init plan-phase returns phase_found false when neither directory nor ROADMAP entry exists', () => {
    const result = runGtdTools('init plan-phase 99', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('init commands ignore archived phases from prior milestones sharing a number', () => {
  ...
  test('init plan-phase prefers current ROADMAP entry over archived v1.0 phase of same number', () => {
    const result = runGtdTools('init plan-phase 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init verify-work prefers current ROADMAP entry over archived v1.0 phase of same number', () => {
    const result = runGtdTools('init verify-work 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('init plan-phase zero-padded phase number (bug #2391)', () => {
  ...
  test('unpadded "3" and zero-padded "03" return identical phase identity', () => {
    const result3 = runGtdTools('init plan-phase 3', tmpDir);
    const result03 = runGtdTools('init plan-phase 03', tmpDir);
...
describe('cmdInitTodos', () => {
  ...
  test('empty pending dir returns zero count', () => {
    ...
    const result = runGtdTools('init todos', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('area filter returns only matching todos', () => {
    ...
    const result = runGtdTools('init todos backend', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('area filter miss returns zero count', () => {
    ...
    const result = runGtdTools('init todos nonexistent', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitMilestoneOp', () => {
  ...
  test('no phase directories returns zero counts', () => {
    const result = runGtdTools('init milestone-op', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitPhaseOp fallback', () => {
  ...
  test('normal path with existing directory', () => {
    ...
    const result = runGtdTools('init phase-op 3', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('fallback to ROADMAP when no directory exists', () => {
    ...
    const result = runGtdTools('init phase-op 5', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runGtdTools('init phase-op 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('neither directory nor roadmap entry returns not found', () => {
    ...
    const result = runGtdTools('init phase-op 99', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitQuick', () => {
  ...
  test('with description generates slug and task_dir with YYMMDD-xxx format', () => {
    const result = runGtdTools('init quick "Fix login bug"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('without description returns null slug and task_dir', () => {
    const result = runGtdTools('init quick', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('two rapid calls produce different quick_ids (no collision within 2s window)', () => {
    ...
    const r1 = runGtdTools('init quick "Task one"', tmpDir);
    const r2 = runGtdTools('init quick "Task two"', tmpDir);
  ...
  test('long description truncates slug to 40 chars', () => {
    const result = runGtdTools('init quick "This is a very long description that should get truncated to forty characters maximum"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns quick branch name when quick_branch_template is configured', () => {
    fs.writeFileSync(
      ...
      JSON.stringify({
        git: {
          quick_branch_template: 'gtd/quick-{num}-{slug}',
        },
      }, null, 2)
    );
    ...
    assert.ok(output.branch_name.startsWith('gtd/quick-'));
  ...
  test('uses fallback slug in quick branch name when description is omitted', () => {
    fs.writeFileSync(
      ...
      JSON.stringify({
        git: {
          quick_branch_template: 'gtd/quick-{quick}-{slug}',
        },
      }, null, 2)
    );
...
describe('cmdInitMapCodebase', () => {
  ...
  test('no codebase dir returns empty', () => {
    const result = runGtdTools('init map-codebase', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('map-codebase workflow does not list OpenCode under runtimes without Task tool (#1316)', () => {
    const workflow = fs.readFileSync(
      path.join(__dirname, '..', 'get-tasks-done', 'workflows', 'map-codebase.md'), 'utf8'
    );
...
describe('cmdInitNewProject', () => {
  ...
  test('greenfield project with no code', () => {
    const result = runGtdTools('init new-project', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdInitNewMilestone', () => {
  ...
  test('file existence flags reflect actual state', () => {
    ...
    const result1 = runGtdTools('init new-milestone', tmpDir);
    ...
    const result2 = runGtdTools('init new-milestone', tmpDir);
...
describe('findProjectRoot integration via --cwd', () => {
  ...
  test('init quick from sub-repo CWD returns project_root pointing to parent', () => {
    ...
    const result = runGtdTools(['init', 'quick', 'test task', '--cwd', backendDir]);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('init quick from project root returns project_root as-is', () => {
    const result = runGtdTools(['init', 'quick', 'test task', '--cwd', projectRoot]);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state load from sub-repo CWD reads project root config', () => {
    ...
    const backendDir = path.join(projectRoot, 'backend');
    const result = runGtdTools(['state', '--cwd', backendDir]);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('withProjectRoot project identity', () => {
  ...
  test('injects project_code when config.project_code is set', () => {
    ...
    const result = runGtdTools(['init', 'quick', 'test task'], tmpDir, { HOME: tmpDir });
    assert.ok(result.success, `Command failed: ${result.error}`);
