 ...
 *
 * The legacy `gtd-update-check` hook migration in bin/install.js uses two
 * separate .replace() calls:
 *   1. LF-only regex: /\n# GTD Hooks\n\[\[hooks\]\]\nevent = ...\n/
 *   2. CRLF-only regex: /\r\n# GTD Hooks\r\n\[\[hooks\]\]\r\nevent = ...\r\n/
 *
 * These patterns fail when config.toml has mixed line endings — e.g. the
 * "# GTD Hooks" header uses LF but the body uses CRLF, or vice versa. This
 * can happen when the file is created cross-platform (Windows/Linux), when
 * editors convert only part of the file, or when a previous GTD version wrote
 ...
 *
 * Test approach: write a `.codex/config.toml` with a stale gtd-update-check
...
const { install, GTD_CODEX_MARKER } = require(INSTALL_SRC);
...
describe('#2698: CRLF stale gtd-update-check block is removed on Codex reinstall', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gtd-crlf-install-2698-'));
  });
  ...
  // Helper: pre-populate .codex/config.toml with a GTD marker + stale hooks block
...
function writeCodexConfigWithStaleHooks(dir, headerEol, bodyEol) {
    // Build the stale block with header EOL for the "# GTD Hooks" line, but body EOL
    // for the content lines (simulates a file edited by two different platforms).
    const staleBlock = [
      '# GTD Hooks',           // line that starts the stale section
      '[[hooks]]',
      'event = "SessionStart"',
      'command = "node /old/path/gtd-update-check.js"',
    ].join(bodyEol);

    // Put the stale block in user content BEFORE the GTD marker. The GTD marker area
    ...
    const content = [
      ...
      'codex_hooks = true',
      '',
    ].join(headerEol) + headerEol + staleBlock + headerEol + headerEol + GTD_CODEX_MARKER + headerEol;
  ...
  test('LF config.toml: stale gtd-update-check block removed on reinstall', (t) => {
    ...
    assert.ok(
      !content.includes('gtd-update-check'),
      'Stale gtd-update-check entry must be removed from LF config.toml (#2698)'
    );
    const hooksJsonCommands = readHooksSessionStartCommands(path.join(tmpDir, '.codex'));
    assert.equal(
      hooksJsonCommands.some((cmd) => cmd.includes('gtd-check-update')),
      true,
      'New gtd-check-update hook must appear in hooks.json after reinstall'
    );
  });

  test('CRLF config.toml: stale gtd-update-check block removed on reinstall', (t) => {
    const origCwd = process.cwd();
    t.after(() => { process.chdir(origCwd); });
    process.chdir(tmpDir);
    ...
    assert.ok(
      !content.includes('gtd-update-check'),
      'Stale gtd-update-check entry must be removed from CRLF config.toml (#2698)'
    );
    const hooksJsonCommands = readHooksSessionStartCommands(path.join(tmpDir, '.codex'));
    assert.equal(
      hooksJsonCommands.some((cmd) => cmd.includes('gtd-check-update')),
  ...
  test('mixed-EOL config.toml: stale block with LF header but CRLF body removed on reinstall', (t) => {
    ...
    assert.ok(
      !content.includes('gtd-update-check'),
      [
        'Stale gtd-update-check block with mixed LF/CRLF endings must be removed (#2698).',
