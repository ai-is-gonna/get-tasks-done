...
describe('analyze-dependencies command', () => {
  test('analyze-dependencies command file was consolidated away (deleted in #2790)', () => {
    // The standalone /gtd-analyze-dependencies command was removed as a dead skill in #2790.
    // The underlying workflow (workflows/analyze-dependencies.md) remains functional.
    const deleted = path.join(__dirname, '..', 'commands', 'gtd', 'analyze-dependencies.md');
  ...
  test('workflow file is sufficient without a standalone command file', () => {
    const p = path.join(__dirname, '..', 'get-tasks-done', 'workflows', 'analyze-dependencies.md');
  ...
  test('docs/COMMANDS.md does not document the consolidated-away /gtd-analyze-dependencies entry', () => {
    ...
    // from consolidated entry points (see gtd-phase / gtd-progress workflow chains).
    ...
    assert.ok(!/^### `\/gtd-analyze-dependencies`/m.test(content),
      'COMMANDS.md should not document the removed /gtd-analyze-dependencies command');
  });
});
