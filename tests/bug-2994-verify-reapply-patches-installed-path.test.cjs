 ...
 * not installed at ${GTD_HOME}/scripts/.
 *
 * Root cause: bin/install.js copies the get-tasks-done/ source tree to
 * ${configDir}/get-tasks-done/ but does NOT copy the top-level scripts/
 * directory. The verifier script lived under scripts/ so /gtd-reapply-patches
 * Step 5 hit `Cannot find module …/scripts/verify-reapply-patches.cjs`.
 *
 * Fix: move the script to get-tasks-done/bin/verify-reapply-patches.cjs
...
const RUNTIME_SCRIPT_PATH = path.join(ROOT, 'get-tasks-done', 'bin', 'verify-reapply-patches.cjs');
const STALE_SCRIPT_PATH = path.join(ROOT, 'scripts', 'verify-reapply-patches.cjs');
const REAPPLY_WORKFLOW = path.join(ROOT, 'get-tasks-done', 'workflows', 'reapply-patches.md');

describe('Bug #2994: verify-reapply-patches.cjs lives at the runtime-installed path', () => {
  test('the script exists under get-tasks-done/bin/ (installed by copyWithPathReplacement)', () => {
    assert.equal(fs.existsSync(RUNTIME_SCRIPT_PATH), true,
      `Expected verifier script at ${RUNTIME_SCRIPT_PATH} -- installer copies get-tasks-done/ recursively`);
  });

  test('the script does NOT live at the legacy scripts/ path (not installed)', () => {
    assert.equal(fs.existsSync(STALE_SCRIPT_PATH), false,
      `scripts/ is not copied by installer; verifier must be under get-tasks-done/bin/ instead`);
  });
...
// Parse reapply-patches.md to extract every `node "${GTD_HOME}/...cjs"`
...
function extractScriptInvocations(markdown) {
  const invocations = [];
  const re = /node\s+"\$\{GTD_HOME\}\/([^"]+\.cjs)"/g;
  let match;
...
describe('Bug #2994: reapply-patches workflow references the runtime-installed path', () => {
  test('every node ${GTD_HOME}/... invocation in reapply-patches.md uses an installed runtime path', () => {
    const md = fs.readFileSync(REAPPLY_WORKFLOW, 'utf-8');
    const invocations = extractScriptInvocations(md);
    assert.ok(invocations.length > 0, 'sanity: expected at least one node ${GTD_HOME}/... invocation in reapply-patches.md');

    const violations = invocations.filter(inv => !inv.relPath.startsWith('get-tasks-done/'));
    assert.deepEqual(violations, [], `invocations under non-installed paths: ${JSON.stringify(violations)}`);
  });

  test('reapply-patches.md references the verifier at get-tasks-done/bin/verify-reapply-patches.cjs', () => {
    const md = fs.readFileSync(REAPPLY_WORKFLOW, 'utf-8');
    const invocations = extractScriptInvocations(md);
    ...
    assert.deepEqual(
      ...
      ['get-tasks-done/bin/verify-reapply-patches.cjs'],
