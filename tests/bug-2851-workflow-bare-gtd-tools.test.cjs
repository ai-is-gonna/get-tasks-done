/**
 * Bug #2851: plan-phase.md §13e calls bare `gtd-tools` — incomplete fix of #2245
 *
 * `gtd-tools` is NOT a published bin entry. The shipped invocation pattern is:
 *
 *   node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" <subcommand> [args]
 *
 * Some workflow markdown files leaked the bare `gtd-tools <subcommand>` form,
 * which fails with `command not found` at runtime.
 *
 * This test parses every markdown file in get-tasks-done/workflows/ structurally:
 * it tokenizes the content into fenced code blocks, then on each shell-block
 * line checks whether `gtd-tools` appears as a bare command (not preceded by
 * `node `, not part of the filename `gtd-tools.cjs`, not inside a comment).
 *
...
/**
 * Check a single shell-block line for a bare `gtd-tools` command-position token.
 * Returns true if the line is a violation.
 */
function lineHasBareGtdTools(line) {
  ...
  // as a fresh command, so a bare `gtd-tools` inside them is just as broken
  ...
  // Then walk tokens — a violation is a token that starts with `gtd-tools`
  // followed by a word boundary (so `gtd-tools.cjs` does NOT match), and the
  // preceding token is NOT `node`.
  const segments = l.split(/(?:\s*(?:&&|\|\||;|\|)\s*)/);
  for (const seg of segments) {
    const tokens = seg.trim().split(/\s+/).filter(Boolean);
    if (tokens.length === 0) continue;
    // Skip env var assignments at the start (FOO=bar gtd-tools …, tmp=1 gtd-tools …).
    ...
    // Match `gtd-tools` exactly (no extension), as command position.
    if (cmd === 'gtd-tools') return true;
  }
  return false;
}

describe('bug-2851: workflow files must not call bare `gtd-tools` (#2245 sweep regression)', () => {
  test('no get-tasks-done/workflows/*.md file contains a bare gtd-tools command', () => {
    ...
    for (const f of files) {
      ...
      for (const blk of blocks) {
        for (let i = 0; i < blk.lines.length; i++) {
          if (lineHasBareGtdTools(blk.lines[i])) {
    ...
    assert.deepStrictEqual(
      violations,
      [],
      'Bare `gtd-tools` invocations found in workflow shell blocks. ' +
        'Use `node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" <subcommand>` instead.\n' +
        violations.join('\n'),
    );
  });
    ...
    for (const blk of blocks) {
      for (const line of blk.lines) {
        if (/gap-analysis/.test(line) && !/^\s*#/.test(line)) {
          ...
          assert.match(
            line,
            /\bnode\s+["']?\$HOME\/\.claude\/get-tasks-done\/bin\/gtd-tools\.cjs["']?\s+gap-analysis\b/,
