...
/**
 * Tests for `/gtd-settings-advanced` — power-user configuration command (#2528).
 *
 * Covers:
 ...
 *   - /gtd-settings confirmation output advertises /gtd-settings-advanced
...
const COMMAND_PATH = path.join(ROOT, 'commands', 'gtd', 'config.md');
const WORKFLOW_PATH = path.join(ROOT, 'get-tasks-done', 'workflows', 'settings-advanced.md');
const SETTINGS_WORKFLOW_PATH = path.join(ROOT, 'get-tasks-done', 'workflows', 'settings.md');
...
const SPEC_FIELDS = {
  ...
  git: [
    ...
    { key: 'git.phase_branch_template',      default: 'gtd/phase-{phase}-{slug}' },
    { key: 'git.milestone_branch_template',  default: 'gtd/{milestone}-{slug}' },
  ],
  runtime: [
...
describe('gtd-settings-advanced — file scaffolding', () => {
  ...
  test('workflow file exists at get-tasks-done/workflows/settings-advanced.md', () => {
  ...
  test('command frontmatter has name, description, allowed-tools', () => {
    ...
    const fm = fmMatch[1];
    assert.match(fm, /name:\s*gtd:config/, 'frontmatter missing name (gtd:config)');
...
describe('gtd-settings-advanced — workflow structure', () => {
  let workflow;
  try {
...
describe('gtd-settings-advanced — VALID_CONFIG_KEYS coverage', () => {
  for (const key of ALL_SPEC_KEYS) {
    test(`VALID_CONFIG_KEYS contains "${key}"`, () => {
      assert.ok(
        VALID_CONFIG_KEYS.has(key),
        `VALID_CONFIG_KEYS missing ${key} — add it to get-tasks-done/bin/lib/config-schema.cjs`
      );
    });
  }
});

// ─── /gtd-settings mentions /gtd-settings-advanced ────────────────────────────

describe('/gtd-settings advertises /gtd-settings-advanced', () => {
  test('settings workflow mentions canonical /gtd-config --advanced', () => {
    const text = fs.readFileSync(SETTINGS_WORKFLOW_PATH, 'utf-8');
    assert.ok(
      text.includes('/gtd:config --advanced'),
      'get-tasks-done/workflows/settings.md must mention /gtd:config --advanced'
    );
    assert.ok(
      !text.includes('gtd-settings-advanced') && !text.includes('gtd:settings-advanced'),
      'get-tasks-done/workflows/settings.md must not mention legacy /gtd-settings-advanced variants'
    );
  });
});
...
describe('gtd-settings-advanced — config merge preserves unrelated keys', () => {
...
describe('gtd-settings-advanced — negative scenarios', () => {
  ...
  // The /gtd-settings-advanced workflow promises non-numeric input is never
  ...
  test('config-set on a numeric key stores non-numeric input verbatim as string (workflow layer must reject before reaching here)', (t) => {
    ...
    // /gtd-settings-advanced workflow. If a future change adds a numeric
