...
/**
 * SDK CLI integration tests for `gtd-tools validate context`.
 *
...
describe('gtd-tools validate context — CLI argument errors', () => {
  test('missing --tokens-used fails with named flag in stderr', () => {
    const r = runGtdTools(['validate', 'context', '--context-window', '200000']);
    assert.strictEqual(r.success, false);
    assert.match(r.error, /tokens-used/i);
  });

  test('missing --context-window fails with named flag in stderr', () => {
    const r = runGtdTools(['validate', 'context', '--tokens-used', '100000']);
    assert.strictEqual(r.success, false);
    assert.match(r.error, /context-window/i);
  });

  test('non-numeric --tokens-used reports the offending flag', () => {
    const r = runGtdTools(['validate', 'context', '--tokens-used', 'abc', '--context-window', '200000']);
    assert.strictEqual(r.success, false);
    assert.match(r.error, /tokens-used/i);
  });

  test('negative --tokens-used reports the offending flag', () => {
    const r = runGtdTools(['validate', 'context', '--tokens-used', '-1', '--context-window', '200000']);
    assert.strictEqual(r.success, false);
    assert.match(r.error, /tokens-used/i);
  });
});

describe('gtd-tools validate context — JSON vs human rendering', () => {
  test('--json emits the classifier result plus a recommendation field', () => {
    ...
    const r = runGtdTools(['validate', 'context', '--tokens-used', '50000', '--context-window', '200000', '--json']);
  ...
  test('human mode (default) prints percent, state, and recommendation', () => {
    const r = runGtdTools(['validate', 'context', '--tokens-used', '140000', '--context-window', '200000']);
    assert.strictEqual(r.success, true);
    assert.match(r.output, /70%/);
    assert.match(r.output, /critical/);
    assert.match(r.output, /\/gtd-thread/);
  });

  test('human mode omits the recommendation line for healthy state', () => {
    const r = runGtdTools(['validate', 'context', '--tokens-used', '40000', '--context-window', '200000']);
    assert.strictEqual(r.success, true);
    assert.match(r.output, /20%/);
    assert.match(r.output, /healthy/);
    assert.doesNotMatch(r.output, /\/gtd-thread/, 'healthy output must not nag the user');
  });
});

describe('gtd-tools validate context — recommendation copy per state', () => {
  ...
  test('warning state recommends /gtd-thread', () => {
    const r = runGtdTools(['validate', 'context', '--tokens-used', '130000', '--context-window', '200000', '--json']);
    const obj = JSON.parse(r.output);
    assert.strictEqual(obj.state, 'warning');
    assert.match(obj.recommendation, /\/gtd-thread/);
  });

  test('critical state names the fracture-point reasoning risk', () => {
    const r = runGtdTools(['validate', 'context', '--tokens-used', '160000', '--context-window', '200000', '--json']);
    const obj = JSON.parse(r.output);
