...
/**
 * Regression test for #3197 — gtd-tools config-set rejects workflow._auto_chain_active.
 *
 * Root cause: RUNTIME_STATE_KEYS was added to sdk/src/query/config-schema.ts in #3162
 * but not to get-tasks-done/bin/lib/config-schema.cjs, so gtd-tools.cjs users still hit
...
describe('#3197 — gtd-tools.cjs config-set workflow._auto_chain_active', () => {
  test('config-set workflow._auto_chain_active true succeeds via gtd-tools.cjs (CJS path)', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));

    const result = runGtdTools(['config-set', 'workflow._auto_chain_active', 'true'], tmpDir);
    assert.ok(
      result.success,
  ...
  test('config-set workflow._auto_chain_active true writes value to config.json', (t) => {
    ...
    runGtdTools(['config-set', 'workflow._auto_chain_active', 'true'], tmpDir);
  ...
  test('config-set workflow._auto_chain_active false writes false to config.json', (t) => {
    ...
    runGtdTools(['config-set', 'workflow._auto_chain_active', 'false'], tmpDir);
