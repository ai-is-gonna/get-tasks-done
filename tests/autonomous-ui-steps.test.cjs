...
const WORKFLOW_PATH = path.join(__dirname, '..', 'get-tasks-done', 'workflows', 'autonomous.md');
...
describe('autonomous workflow ui-phase and ui-review integration (#1375)', () => {
  ...
  describe('step 3a.5 — UI design contract before planning', () => {
    ...
    test('UI design contract step invokes gtd:ui-phase skill', () => {
      assert.ok(
        content.includes('skill="gtd-ui-phase"'),
        'should invoke gtd-ui-phase via Skill()'
      );
    });
  ...
  describe('step 3d.5 — UI review after execution', () => {
    ...
    test('UI review step invokes gtd:ui-review skill', () => {
      assert.ok(
        content.includes('skill="gtd-ui-review"'),
        'should invoke gtd-ui-review via Skill()'
      );
    });
