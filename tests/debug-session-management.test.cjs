...
describe('debug session management implementation', () => {
  test('DEBUG.md template contains reasoning_checkpoint field', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'get-tasks-done/templates/DEBUG.md'),
      'utf8'
    );
  ...
  test('debug command contains list subcommand logic', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'get-tasks-done/workflows/debug.md'),
      'utf8'
    );
    assert.ok(
  ...
  test('gtd-debugger contains structured reasoning checkpoint', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'agents/gtd-debugger.md'),
      'utf8'
    );
    assert.ok(content.includes('reasoning_checkpoint'), 'gtd-debugger.md must contain reasoning_checkpoint');
  });

  test('gtd-debugger contains TDD checkpoint mode', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'agents/gtd-debugger.md'),
      'utf8'
    );
    assert.ok(content.includes('tdd_mode'), 'gtd-debugger.md must contain tdd_mode');
    assert.ok(content.includes('TDD CHECKPOINT'), 'gtd-debugger.md must contain TDD CHECKPOINT return format');
  });

  test('gtd-debugger contains delta debugging technique', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'agents/gtd-debugger.md'),
      'utf8'
    );
    assert.ok(content.includes('Delta Debugging'), 'gtd-debugger.md must contain Delta Debugging technique');
  });

  test('gtd-debugger contains security note about DATA_START', () => {
    const content = fs.readFileSync(
      path.join(process.cwd(), 'agents/gtd-debugger.md'),
      'utf8'
    );
    assert.ok(content.includes('DATA_START'), 'gtd-debugger.md must contain DATA_START security reference');
  });
});

// Tests for #2148 and #2151
describe('debug skill dispatch and sub-orchestrator (#2148, #2151)', () => {
  test('gtd-debugger ROOT CAUSE FOUND format includes specialist_hint field', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'gtd-debugger.md'), 'utf8');
    assert.ok(content.includes('specialist_hint'), 'gtd-debugger missing specialist_hint in ROOT CAUSE FOUND');
    assert.ok(content.includes('swift_concurrency'), 'gtd-debugger missing specialist_hint derivation guidance');
  });

  test('debug.md orchestrator has specialist skill dispatch step', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'get-tasks-done/workflows/debug.md'), 'utf8');
  ...
  test('gtd-debug-session-manager agent exists with correct tools', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'gtd-debug-session-manager.md'), 'utf8');
    assert.ok(content.includes('Agent'), 'gtd-debug-session-manager missing Agent tool');
    assert.ok(content.includes('AskUserQuestion'), 'gtd-debug-session-manager missing AskUserQuestion tool');
  });

  test('gtd-debug-session-manager spawns debugger with Agent() dispatcher', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'gtd-debug-session-manager.md'), 'utf8');
    assert.ok(content.includes('\nAgent('), 'session manager must dispatch debugger with Agent(');
  });

  test('gtd-debug-session-manager uses DATA_START/DATA_END for checkpoint responses', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'gtd-debug-session-manager.md'), 'utf8');
    assert.ok(content.includes('DATA_START') && content.includes('DATA_END'),
      'gtd-debug-session-manager missing security boundaries on checkpoint responses');
  });

  test('gtd-debug-session-manager has compact summary output format', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'gtd-debug-session-manager.md'), 'utf8');
    assert.ok(content.includes('DEBUG SESSION COMPLETE'), 'session manager missing compact summary format');
  });

  test('gtd-debug-session-manager includes anti-heredoc rule', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'agents', 'gtd-debug-session-manager.md'), 'utf8');
    assert.ok(content.includes('heredoc'), 'session manager missing anti-heredoc rule');
  });

  test('debug.md delegates to gtd-debug-session-manager', () => {
    const content = fs.readFileSync(path.join(process.cwd(), 'get-tasks-done/workflows/debug.md'), 'utf8');
    assert.ok(content.includes('gtd-debug-session-manager'),
