/**
 * Drift-guard for bug #3195: quick.md must use the git-history-based
 * resurrection guard (WAS_DELETED check), not the
 * inverted PRE_MERGE_FILES grep form that deletes brand-new files.
 *
 * The PRE_MERGE_FILES form was fixed in work-task-issue.md by PR #2510 but
...
const QUICK_MD = path.join(
  __dirname, '..', 'get-tasks-done', 'workflows', 'quick.md'
);

describe('resurrection guard drift check — quick.md (#3195)', () => {
  let quickContent;
