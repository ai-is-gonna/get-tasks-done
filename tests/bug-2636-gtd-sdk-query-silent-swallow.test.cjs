/**
 * Regression guard for #2636 — `gtd-sdk query agent-skills <slug>` calls in
 * workflows must NOT silently swallow failures via a bare `2>/dev/null`.
 *
 * Root cause of #2636: when the installed npm `@ai-is-gonna/gtd-sdk` was stale and
 * the `agent-skills` handler was missing, every workflow line of the form
 *   AGENT_SKILLS_X=$(gtd-sdk query agent-skills <slug> 2>/dev/null)
 ...
 *
 * Test scope: ONLY `gtd-sdk query agent-skills …` (the exact noun implicated
 * in #2636). Other `gtd-sdk query config-get …` patterns commonly use
 ...
 *
 * Scans:  get-tasks-done/workflows/**\/*.md  and  commands/**\/*.md
 */
...
const SCAN_ROOTS = [
  path.join(REPO_ROOT, 'get-tasks-done', 'workflows'),
...
describe('bug #2636 — agent-skills query must not silently swallow failures', () => {
  test('no `gtd-sdk query agent-skills ... 2>/dev/null` in workflows', () => {
    const files = [];
    for (const root of SCAN_ROOTS) walk(root, files);
    assert.ok(files.length > 0, 'expected to scan some workflow/command files');

    // Match `gtd-sdk query agent-skills <slug>` followed (on the same line)
    // by `2>/dev/null` — the silent-swallow anti-pattern.
    const ANTI = /gtd-sdk\s+query\s+agent-skills\b[^\n]*2>\/dev\/null/;
    ...
    assert.strictEqual(
      offenders.length, 0,
      'Found `gtd-sdk query agent-skills ... 2>/dev/null` (silent swallow — ' +
