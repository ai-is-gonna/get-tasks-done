...
//   1. Cache-stale: bare `npx -y get-tasks-done@latest` hits npx's local
//      cache and may pull an older version instead of @latest.
//   2. Token-routing: Bash-tool wrappers misroute the `@` token in
//      `get-tasks-done@latest`, causing npm to error with
//      "Unknown command: get-tasks-done@latest".
//
// The robust form is:
//   npx -y --package=get-tasks-done@latest -- get-tasks-done $ARGS
//
...
const UPDATE_WF = path.join(ROOT, 'get-tasks-done', 'workflows', 'update.md');

const src = fs.readFileSync(UPDATE_WF, 'utf8');

test('bug #3130: update.md contains no bare npx invocations (cache-stale form)', () => {
  // Any occurrence of `npx -y get-tasks-done@latest` without `--package=`
  // is the stale form that triggers the two failure modes.
  const stale = (src.match(/npx -y get-tasks-done@latest[^\n]*/g) || []);
  assert.deepEqual(
    stale,
    [],
...
test('bug #3130: update.md has >=3 robust npx invocations (--package= + -- separator)', () => {
  ...
  const robust = (src.match(/npx -y --package=get-tasks-done@latest -- get-tasks-done/g) || []);
  assert.ok(
    robust.length >= 3,
