...
// tests the deployed contract. The commands/gtd/*.md reads in the helper are
 ...
 *   - get-tasks-done/workflows/help.md  (deprecation-trail prose)
...
const ALLOWED_HISTORICAL_MENTIONS = new Set([
  path.join(ROOT, 'get-tasks-done', 'workflows', 'help.md'),
...
// syntax placeholders — they match the /gtd-* regex but are NOT user-typable
...
const INTERNAL_COMPONENT_SLUGS = new Set([
  ...
  // (e.g. "/gtd-command-name [args]"). It is not a registered command.
  'command-name',
  'command',

  // gtd-tools.cjs — the legacy Node CLI binary (bin/gtd-tools.cjs).
  // Docs reference it as a path component in shell examples, not as a slash command.
  // Example: node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" state validate
  'tools',

  // Hook scripts — internal runtime hooks, not user-invocable slash commands.
  //   hooks/gtd-statusline.js       — session statusline hook
  //   hooks/gtd-context-monitor.js  — context-window monitor hook
  //   hooks/gtd-update-banner.js    — update-available banner hook
  //   hooks/gtd-graphify-update.sh  — knowledge-graph auto-update PostToolUse hook (#3347)
  // These appear in docs as file-path references (e.g. "gtd-statusline.js reads
  ...
  // gtd-update-check.json — background update-check CACHE FILE, not a slash command.
  // ARCHITECTURE.md references "~/.cache/gtd/gtd-update-check.json" as a path;
  // the regex captures "/gtd-update-check" from the path component.
  'update-check',

  // Internal agent names referenced in ARCHITECTURE.md tables of agents.
  // These are spawned agents (gtd-planner, etc.), not user-typable slash commands.
  'planner',

  // Malformed token from SDK init reference: "/gtd-init-" appears as a truncated
  // prefix in CLI-TOOLS.md describing the gtd-sdk init command family
  // (e.g., "gtd-sdk query init.phase-op 12"). The regex captures "/gtd-init-"
  // without a following slug — this is a documentation formatting artifact, not
  // a real command token.
  'init-',

  // ai-is-gonna — GitHub organization name. The regex can capture path
  // components from repository URLs; those are not slash commands.
  'get-tasks-done',

  // @ai-is-gonna/gtd-sdk — npm scoped package name. The regex captures
  // "/gtd-sdk" from the package path component; the CLI binary is `gtd-sdk`,
  // not a slash command.
  'sdk',

  // ~/gtd-workspaces/ — filesystem directory path used by /gtd-workspace.
  // Docs reference "~/gtd-workspaces/<name>" as the default workspace directory
  // in shell examples and option tables (e.g. "--path /target (default: ~/gtd-workspaces/<name>)").
  // The regex captures "/gtd-workspaces" from the path component. The LIVE slash
  // command is "/gtd-workspace" (singular) — not "/gtd-workspaces" (plural).
  'workspaces',

  // GTD logo asset path. The regex captures "/gtd-logo-2000" from the image
  // path, but it is a static asset, not a slash command.
  'logo-2000',

  // Smoke-test directory path — locale docs reference "/tmp/gtd-smoke-$(date +%s)"
  // as a temporary directory path in bash code-block examples. The regex captures
  // "/gtd-smoke-" from the filesystem path. Not a slash command.
  'smoke-',

  // Template placeholders. These are not registered commands.
  'alternative-1',
  'alternative-2',
]);
...
function extractCommandTokens(content) {
  ...
  function isInternal(token) {
    // Strip the /gtd- or /gtd: or $gtd- prefix to get the slug
    const slug = token.replace(/^(?:\/gtd[:-]|\$gtd-)/, '');
  ...
  const allSlash = (stripped.match(/\/gtd-[a-z0-9][a-z0-9-]*/g) || []);
  const allColon = (stripped.match(/\/gtd:[a-z0-9][a-z0-9-]*/g) || []);
  const allDollar = (stripped.match(/\$gtd-[a-z0-9][a-z0-9-]*/g) || []);
...
describe('getLiveCommandTokens() — helper contract', () => {
  ...
  test('returns a non-empty set (commands/gtd/ has registered commands)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.size > 0, 'live registry must contain at least one token');
  });

  test('contains /gtd-help (from commands/gtd/help.md name: gtd:help)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/gtd-help'), 'registry must contain /gtd-help');
  });

  test('contains /gtd:help (Gemini form)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/gtd:help'), 'registry must contain /gtd:help');
  });

  test('contains $gtd-help (Codex form)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('$gtd-help'), 'registry must contain $gtd-help');
  });

  test('contains /gtd-plan-phase (from commands/gtd/plan-phase.md)', () => {
    const result = getLiveCommandTokens();
    assert.ok(result.has('/gtd-plan-phase'), 'registry must contain /gtd-plan-phase');
  });

  test('contains exactly 3 tokens per slug (slash, colon, dollar)', () => {
    const result = getLiveCommandTokens();
    // Every /gtd-slug should have a matching /gtd:slug and $gtd-slug
    let tokenCount = 0;
    for (const token of result) {
      if (token.startsWith('/gtd-')) tokenCount++;
    }
    const slashTokens = [...result].filter(t => t.startsWith('/gtd-'));
    for (const slash of slashTokens) {
      const slug = slash.slice('/gtd-'.length);
      assert.ok(
        result.has(`/gtd:${slug}`),
        `registry must contain Gemini form /gtd:${slug} for slash form ${slash}`
      );
      assert.ok(
        result.has(`$gtd-${slug}`),
        `registry must contain Codex form $gtd-${slug} for slash form ${slash}`
      );
    }
  });

  test('does NOT contain removed /gtd-reapply-patches', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/gtd-reapply-patches'), 'registry must NOT contain removed /gtd-reapply-patches');
  });

  test('does NOT contain removed /gtd-code-review-fix', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/gtd-code-review-fix'), 'registry must NOT contain removed /gtd-code-review-fix');
  });

  test('does NOT contain removed /gtd-status', () => {
    const result = getLiveCommandTokens();
    assert.ok(!result.has('/gtd-status'), 'registry must NOT contain removed /gtd-status');
  });
...
describe('getLiveCommandTokens() — fixture contract', () => {
  test('parses gtd:foo frontmatter and emits 3 canonical tokens', () => {
    // This test validates the parsing logic against a known-good fixture
    // by inspecting the live registry for commands/gtd/help.md (name: gtd:help).
    // Fixture file tests are done inline since the helper reads commands/gtd/ only.
    // The canonical token contract:
    //   name: gtd:foo → /gtd-foo, /gtd:foo, $gtd-foo
    const registry = getLiveCommandTokens();
    // We know help.md has name: gtd:help
    const slug = 'help';
    assert.ok(registry.has(`/gtd-${slug}`), `must have /gtd-${slug}`);
    assert.ok(registry.has(`/gtd:${slug}`), `must have /gtd:${slug}`);
    assert.ok(registry.has(`$gtd-${slug}`), `must have $gtd-${slug}`);
  });

  test('parses gtd-slug frontmatter (ns-* commands) and emits 3 tokens', () => {
    // ns-context.md has name: gtd-context (dash-style, no colon)
    const registry = getLiveCommandTokens();
    assert.ok(registry.has('/gtd-context'), 'must have /gtd-context (from ns-context.md)');
    assert.ok(registry.has('/gtd:context'), 'must have /gtd:context (Gemini form)');
    assert.ok(registry.has('$gtd-context'), 'must have $gtd-context (Codex form)');
  });
});
...
describe('adversarial: polarity inversion catches drift deny-list misses', () => {
  test('renaming a live command without updating docs would fail this test (demonstrated via token absence)', () => {
    // If /gtd-progress were renamed to /gtd-status-new, the old /gtd-progress
    // token would not appear in the live registry, and any doc referencing
    // /gtd-progress would fail. The deny-list shape would have passed silently
    ...
    const registry = getLiveCommandTokens();
    assert.ok(registry.has('/gtd-progress'), '/gtd-progress must be live (not renamed to /gtd-status)');
    assert.ok(!registry.has('/gtd-status'), '/gtd-status must be absent (was deleted, replaced by /gtd-progress)');
  });

  test('freshly-deleted command /gtd-check-todos is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/gtd-check-todos'), '/gtd-check-todos must not be in the live registry');
  });

  test('freshly-deleted command /gtd-new-workspace is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/gtd-new-workspace'), '/gtd-new-workspace must not be in the live registry');
  });

  test('freshly-deleted command /gtd-plan-milestone-gaps is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/gtd-plan-milestone-gaps'), '/gtd-plan-milestone-gaps must not be in the live registry');
  });

  test('freshly-deleted command /gtd-research-phase is absent from registry', () => {
    const registry = getLiveCommandTokens();
    assert.ok(!registry.has('/gtd-research-phase'), '/gtd-research-phase must not be in the live registry');
  });
});
