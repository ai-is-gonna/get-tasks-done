...
const os = require('os');
const { runGtdTools, createTempDir, createTempProject, cleanup } = require('./helpers.cjs');
...
describe('scan-sessions command', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('gtd-profile-test-');
  });
...
describe('extract-messages command', () => {
  ...
  test('extracts user messages from synthetic session', () => {
    ...
    const result = runGtdTools(['extract-messages', 'my-project', '--path', sessionsDir, '--raw'], tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
  ...
  test('filters out meta and internal messages', () => {
    ...
    const result = runGtdTools(['extract-messages', 'filter-test', '--path', sessionsDir, '--raw'], tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
...
describe('profile-questionnaire command', () => {
  ...
  test('returns questionnaire structure', () => {
    const result = runGtdTools('profile-questionnaire --raw', tmpDir);
    assert.ok(result.success, `Failed: ${result.error}`);
    const out = JSON.parse(result.output);
