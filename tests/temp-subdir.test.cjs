/**
 * GTD Tools Tests - dedicated temp subdirectory
 *
 * Tests for issue #1975: GTD temp files should use a dedicated
 * subdirectory (path.join(os.tmpdir(), 'gtd')) instead of writing
...
const GTD_TEMP_DIR = path.join(os.tmpdir(), 'gtd');

// ─── Dedicated temp subdirectory ────────────────────────────────────────────

describe('dedicated gtd temp subdirectory', () => {
  describe('output() temp file placement', () => {
    ...
    test('gtd temp subdirectory path is os.tmpdir()/gtd', () => {
      // The GTD_TEMP_DIR constant should resolve to <tmpdir>/gtd
      assert.strictEqual(GTD_TEMP_DIR, path.join(os.tmpdir(), 'gtd'));
    });
  });
  ...
  describe('reapStaleTempFiles with subdirectory', () => {
    ...
    beforeEach(() => {
      testPrefix = `gtd-tempsub-test-${Date.now()}-`;
      // Ensure the gtd subdirectory exists for test setup
      fs.mkdirSync(GTD_TEMP_DIR, { recursive: true });
    });

    test('removes stale files from gtd subdirectory', () => {
      const stalePath = path.join(GTD_TEMP_DIR, `${testPrefix}stale.json`);
      ...
      assert.ok(!fs.existsSync(stalePath), 'stale file in gtd subdir should be removed');
    });

    test('preserves fresh files in gtd subdirectory', () => {
      const freshPath = path.join(GTD_TEMP_DIR, `${testPrefix}fresh.json`);
      fs.writeFileSync(freshPath, '{}');

      reapStaleTempFiles(testPrefix, { maxAgeMs: 5 * 60 * 1000 });

      assert.ok(fs.existsSync(freshPath), 'fresh file in gtd subdir should be preserved');
      // Clean up
      fs.unlinkSync(freshPath);
    });

    test('removes stale directories from gtd subdirectory', () => {
      const staleDir = path.join(GTD_TEMP_DIR, `${testPrefix}dir`);
      ...
      assert.ok(!fs.existsSync(staleDir), 'stale directory in gtd subdir should be removed');
    });

    test('creates gtd subdirectory if it does not exist', () => {
      // Use a unique nested path to avoid interfering with other tests
      const uniqueSubdir = path.join(os.tmpdir(), `gtd-creation-test-${Date.now()}`);
      ...
      assert.doesNotThrow(() => {
        reapStaleTempFiles(`gtd-creation-test-${Date.now()}-`, { maxAgeMs: 0 });
      });
    });

    test('does not scan system tmpdir root for gtd- files', () => {
      ...
      // because it now only scans the gtd subdirectory
      reapStaleTempFiles(testPrefix, { maxAgeMs: 5 * 60 * 1000 });
