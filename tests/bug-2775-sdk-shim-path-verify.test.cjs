 ...
 *
 * `npx get-tasks-done@latest --global` runs the installer, which prints
 * `✓ GTD SDK ready` even though the secondary `gtd-sdk` bin is not on the
 ...
 * — a strictly weaker invariant than `command -v gtd-sdk` resolving.
 *
 * The fix tightens the success gate: after confirming the dist is present,
 * the installer must verify `gtd-sdk` resolves on PATH. If it does not, the
 * installer attempts to materialize the shim into a user-writable PATH
 * location (`~/.local/bin/gtd-sdk`) and re-checks. Only when the PATH probe
 * succeeds does it print `✓ GTD SDK ready`. Otherwise it emits a clear
 ...
 * with a `gtd-sdk` shim. The legacy code printed the success line in this
...
describe('bug #2775: installSdkIfNeeded must verify gtd-sdk on PATH before reporting ready', () => {
  let tmpRoot;
  let sdkDir;
  let pathDir;
  let homeDir;
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('gtd-2775-');
    ...
    // PATH does NOT contain anything with a gtd-sdk shim — simulates npx-cache.
  ...
  test('does NOT print "GTD SDK ready" when gtd-sdk is not callable on PATH and cannot be linked', () => {
    // Make ~/.local/bin not on PATH and not creatable-friendly: PATH stays
    // as a single dir with no gtd-sdk. The installer may attempt to create
    // ~/.local/bin/gtd-sdk, but that location isn't on PATH either, so the
    ...
    const hasReady = /GTD SDK ready/.test(combined);
    const mentionsPath = /not on (your )?PATH|gtd-sdk.*PATH|PATH.*gtd-sdk/i.test(combined);
    assert.ok(
      !hasReady,
      `installer must not print "GTD SDK ready" when gtd-sdk is not on PATH. Output:\n${combined}`,
    );
    assert.ok(
      mentionsPath,
      `installer must surface a PATH-related warning when gtd-sdk is not callable. Output:\n${combined}`,
    );
  });

  test('DOES print "GTD SDK ready" after self-linking into a directory that IS on PATH', () => {
    ...
    assert.ok(
      ...
      `installer must print "GTD SDK ready" after self-linking to a dir on PATH. Output:\n${combined}`,
    );
  ...
  test('symlink-fallback writes a wrapper that require()s the real shim by absolute path (preserves __dirname)', () => {
    ...
    // The fallback must NOT copy bin/gtd-sdk.js into ~/.local/bin (which would
    ...
    const realShimSrc = path.resolve(__dirname, '..', 'bin', 'gtd-sdk.js');
    ...
    const target = path.join(localBin, 'gtd-sdk');
    assert.ok(fs.existsSync(target), `fallback must materialize ${target}`);
    // Critical: it must NOT be a verbatim copy of bin/gtd-sdk.js.
    const targetContent = fs.readFileSync(target, 'utf8');
    const realShimContent = fs.readFileSync(realShimSrc, 'utf8');
    assert.notStrictEqual(
      targetContent,
      realShimContent,
      'fallback must not copyFileSync bin/gtd-sdk.js — that breaks __dirname-based CLI resolution',
    );
  ...
  test('self-link prefers a PATH-backed HOME dir over ~/.local/bin when ~/.local/bin is off-PATH', () => {
    ...
    assert.ok(
      fs.existsSync(path.join(homeBin, 'gtd-sdk')),
      `installer must materialize the link in the on-PATH HOME dir (~/bin), not ~/.local/bin`,
    );
  });

  test('DOES print "GTD SDK ready" when gtd-sdk is already resolvable on PATH', () => {
    // Pre-populate PATH with a `gtd-sdk` shim so the probe finds one.
    const preexisting = path.join(pathDir, 'gtd-sdk');
    ...
    assert.ok(
      ...
      `installer must print "GTD SDK ready" when gtd-sdk is already on PATH. Output:\n${stdout}`,
    );
  });
});
