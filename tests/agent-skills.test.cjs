/**
 * GTD Tools Tests - Agent Skills Injection
 *
...
describe('agent-skills command', () => {
  ...
  test('returns empty when no config exists', () => {
    ...
    const result = runGtdTools(['agent-skills', 'gtd-task-executor'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
  ...
  test('returns empty for unconfigured agent type', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'gtd-task-executor': ['skills/test-skill'],
      },
    });
    const result = runGtdTools(['agent-skills', 'gtd-planner'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.strictEqual(result.output, '');
  });
  ...
  test('returns formatted block for configured agent with single string path', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'gtd-task-executor': 'skills/my-skill',
      },
    });
  ...
  test('handles multiple skill paths', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'gtd-task-executor': ['skills/skill-a', 'skills/skill-b'],
      },
    });
  ...
  test('warns for nonexistent skill path but does not error', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'gtd-task-executor': ['skills/nonexistent'],
      },
    });
  ...
  test('returns empty when no agent type argument provided', () => {
    const result = runGtdTools(['agent-skills'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
...
describe('config-set agent_skills', () => {
  ...
  test('can set agent_skills via dot notation', () => {
    const result = runGtdTools(
      ['config-set', 'agent_skills.gtd-task-executor', '["skills/my-skill"]'],
      tmpDir,
    ...
    assert.deepStrictEqual(
      config.agent_skills['gtd-task-executor'],
      ['skills/my-skill'],
...
describe('agent-skills global: prefix', () => {
  ...
  beforeEach(() => {
    ...
    fakeHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'gtd-1992-home-'));
  ...
  test('global:valid-skill resolves to $HOME/.claude/skills/valid-skill/SKILL.md', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'gtd-task-executor': ['global:valid-skill'] },
    });

    const result = runGtdTools(['agent-skills', 'gtd-task-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome });
  ...
  test('global:invalid!name is rejected by regex and skipped', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'gtd-task-executor': ['global:invalid!name'] },
    });
  ...
  test('global:missing-skill is skipped when directory is absent', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'gtd-task-executor': ['global:missing-skill'] },
    });
  ...
  test('mix of global: and project-relative paths both resolve correctly', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'gtd-task-executor': ['global:shadcn', 'skills/local-skill'] },
    });
  ...
  test('global: with empty name produces clear warning and skips', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'gtd-task-executor': ['global:'] },
    });

    const result = runGtdTools(['agent-skills', 'gtd-task-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome });
    assert.strictEqual(result.output, '', 'should skip empty global: prefix');
    // The warning goes to stderr — cannot assert on it through runGtdTools's output field,
