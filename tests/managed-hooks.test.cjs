 ...
 *
 * gtd-check-update.js contains a MANAGED_HOOKS array used to detect stale
 * hooks after a GTD update. It must list every hook file that GTD ships so
 * that all deployed hooks are checked for staleness — not just the .js ones.
 *
 * The original bug: the 3 bash hooks (gtd-phase-boundary.sh,
 * gtd-session-state.sh, gtd-validate-commit.sh) were missing from
...
const MANAGED_HOOKS_FILE = path.join(HOOKS_DIR, 'gtd-check-update-worker.js');
...
describe('bug #2136: MANAGED_HOOKS must include all shipped hook files', () => {
  ...
  assert.ok(match, 'MANAGED_HOOKS array not found in gtd-check-update-worker.js');
  ...
  // List all GTD-managed hook files in hooks/ (names starting with "gtd-")
  shippedHooks = fs.readdirSync(HOOKS_DIR)
    .filter(f => f.startsWith('gtd-') && (f.endsWith('.js') || f.endsWith('.sh')));

  test('every shipped gtd-*.js hook is in MANAGED_HOOKS', () => {
    const jsHooks = shippedHooks.filter(f => f.endsWith('.js'));
    for (const hookFile of jsHooks) {
      assert.ok(
        managedHooks.includes(hookFile),
        `${hookFile} is shipped in hooks/ but missing from MANAGED_HOOKS in gtd-check-update-worker.js`
      );
    }
  });

  test('every shipped gtd-*.sh hook is in MANAGED_HOOKS', () => {
