/**
 * Bug #2767: Workflows pass paths positionally to `gtd-sdk query commit`.
 *
 ...
 *
 * Primary test: invoke the actual `gtd-sdk query commit[-to-subrepo]` binary
...
/**
 * Invoke `gtd-sdk query <subcommand> <...args>` against a project dir.
...
describe('bug #2767 (behavioral): gtd-sdk query commit --files', () => {
  let tmpDir;

  beforeEach(() => {
    tmpDir = createTempGitProject('gtd-2767-');
...
describe('bug #2767 (behavioral): commit-to-subrepo requires --files', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempGitProject('gtd-2767-sub-');
    fs.writeFileSync(
...
// rendered into chat, not invoked by gtd-tools.cjs), so a textual guard is
// the only available enforcement layer.

const SCAN_DIRS = ['agents', 'commands', 'get-tasks-done', 'docs', 'scripts'];
...
function extractInvocations(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const m = lines[i].match(/(gtd-sdk\s+query\s+commit(?:-to-subrepo)?\b.*)$/);
    if (!m) continue;
    const tail = m[1].match(/^gtd-sdk\s+query\s+commit(?:-to-subrepo)?(.?)/);
...
describe('bug #2767 (supplementary doc-lint): no positional invocations in shipped .md', () => {
  ...
  test('every gtd-sdk query commit invocation either uses --files or has no path args', () => {
    ...
    if (broken.length > 0) {
      ...
      assert.fail(
        `${broken.length} \`gtd-sdk query commit\` invocation(s) pass paths positionally.\n` +
