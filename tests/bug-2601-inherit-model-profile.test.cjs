 ...
 * Object.keys(MODEL_PROFILES['gtd-planner']), which does not include 'inherit'.
...
describe('bug #2601: config-set-model-profile accepts inherit', () => {
  test('config-set-model-profile inherit succeeds', (t) => {
    ...
    const result = runGtdTools(['config-set-model-profile', 'inherit'], tmpDir);
  ...
  test('config-set model_profile inherit succeeds', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));
    const result = runGtdTools(['config-set', 'model_profile', 'inherit'], tmpDir);
  ...
  test('config-set-model-profile inherit writes inherit to config', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));
    runGtdTools(['config-set-model-profile', 'inherit'], tmpDir);
    const getResult = runGtdTools(['config-get', 'model_profile'], tmpDir);
  ...
  test('config-set-model-profile still rejects truly invalid profiles', (t) => {
    const tmpDir = createTempProject();
    t.after(() => cleanup(tmpDir));
    const result = runGtdTools(['config-set-model-profile', 'not-a-real-profile'], tmpDir);
