...
const REFS_DIR = path.join(__dirname, '..', 'get-tasks-done', 'references', 'few-shot-examples');
...
describe('few-shot calibration examples', () => {
  ...
  describe('agent files reference few-shot examples', () => {
    test('gtd-plan-checker.md contains reference to plan-checker few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'gtd-plan-checker.md'));
      assert.match(content, /@~\/\.claude\/get-tasks-done\/references\/few-shot-examples\/plan-checker\.md/);
    });

    test('gtd-verifier.md contains reference to verifier few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'gtd-verifier.md'));
      assert.match(content, /@~\/\.claude\/get-tasks-done\/references\/few-shot-examples\/verifier\.md/);
    });
  });
