...
const { createTempDir, cleanup, runGtdTools } = require('./helpers.cjs');
...
const {
  ...
  slugify,
  zeroPad,
} = require('../get-tasks-done/bin/lib/gtd2-import.cjs');

// ─── Fixture Builders ──────────────────────────────────────────────────────

/** Build a minimal but complete GTD-2 .gtd/ directory in tmpDir. */
function makeGtd2Project(tmpDir, opts = {}) {
  const gtdDir = path.join(tmpDir, '.gtd');
  const m001Dir = path.join(gtdDir, 'milestones', 'M001');
  ...
  fs.writeFileSync(path.join(gtdDir, 'PROJECT.md'), '# My Project\n\nA test project.\n');
  fs.writeFileSync(path.join(gtdDir, 'REQUIREMENTS.md'), [
    '# Requirements',
    '',
    '## Active',
    '',
...
/** Build a two-milestone GTD-2 project. */
function makeTwoMilestoneProject(tmpDir) {
  const gtdDir = path.join(tmpDir, '.gtd');
  const m001Dir = path.join(gtdDir, 'milestones', 'M001');
  const m002Dir = path.join(gtdDir, 'milestones', 'M002');
  ...
  fs.writeFileSync(path.join(gtdDir, 'PROJECT.md'), '# Multi-milestone Project\n');
...
describe('parseGtd2', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempDir('gtd2-parse-'); });
  afterEach(() => { cleanup(tmpDir); });

  test('reads project and requirements passthroughs', () => {
    const gtdDir = makeGtd2Project(tmpDir);
    const data = parseGtd2(gtdDir);
  ...
  test('handles missing .gtd/milestones/ gracefully', () => {
    const gtdDir = path.join(tmpDir, '.gtd');
    fs.mkdirSync(gtdDir, { recursive: true });
    fs.writeFileSync(path.join(gtdDir, 'PROJECT.md'), '# Empty\n');
    const data = parseGtd2(gtdDir);
...
describe('buildPlanningArtifacts', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempDir('gtd2-artifacts-'); });
  afterEach(() => { cleanup(tmpDir); });
  ...
  test('S02 (pending) maps to phase 02 with only CONTEXT and PLAN', () => {
    const gtdDir = makeGtd2Project(tmpDir, { withS02Dir: true });
    const data = parseGtd2(gtdDir);
    const artifacts = buildPlanningArtifacts(data);
  ...
  test('SUMMARY.md strips GTD-2 frontmatter and adds v1 frontmatter', () => {
    const gtdDir = makeGtd2Project(tmpDir);
    const data = parseGtd2(gtdDir);
    const artifacts = buildPlanningArtifacts(data);
    ...
    // GTD-2 frontmatter field should not appear
  ...
  test('multi-milestone: slices numbered sequentially across milestones', () => {
    const gtdDir = makeTwoMilestoneProject(tmpDir);
    const data = parseGtd2(gtdDir);
    const artifacts = buildPlanningArtifacts(data);
...
describe('gtd-tools from-gtd2 CLI', () => {
  let tmpDir;
  beforeEach(() => { tmpDir = createTempDir('gtd2-cli-'); });
  afterEach(() => { cleanup(tmpDir); });

  test('--dry-run returns preview without writing files', () => {
    makeGtd2Project(tmpDir);
    const result = runGtdTools(['from-gtd2', '--dry-run', '--raw'], tmpDir);
  ...
  test('writes .planning/ directory with correct structure', () => {
    makeGtd2Project(tmpDir);
    const result = runGtdTools(['from-gtd2', '--raw'], tmpDir);
    assert.ok(result.success, result.error);
    const parsed = JSON.parse(result.output);
  ...
  test('errors when no .gtd/ directory present', () => {
    const result = runGtdTools(['from-gtd2', '--raw'], tmpDir);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.success, false);
    assert.ok(parsed.error.includes('No .gtd/'));
  });
  ...
  test('--force overwrites existing .planning/', () => {
    ...
    const result = runGtdTools(['from-gtd2', '--force', '--raw'], tmpDir);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.success, true);
    assert.ok(fs.existsSync(path.join(tmpDir, '.planning', 'ROADMAP.md')));
  });
  ...
  test('--path resolves target directory', () => {
    ...
    makeGtd2Project(projectDir);
    // Run from tmpDir but point at projectDir
    const result = runGtdTools(['from-gtd2', '--path', projectDir, '--dry-run', '--raw'], tmpDir);
    assert.ok(result.success, result.error);
    const parsed = JSON.parse(result.output);
    assert.strictEqual(parsed.dryRun, true);
    assert.ok(parsed.preview.includes('PROJECT.md'));
  ...
  test('completion state: S01 done → [x] in ROADMAP.md', () => {
    makeGtd2Project(tmpDir);
    runGtdTools(['from-gtd2', '--raw'], tmpDir);
  ...
  test('SUMMARY.md written for completed task, not for pending', () => {
    makeGtd2Project(tmpDir, { withS02Dir: true });
    runGtdTools(['from-gtd2', '--raw'], tmpDir);
