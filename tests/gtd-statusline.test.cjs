/**
 * Tests for gtd-statusline.js GTD state display helpers.
 *
 * Covers:
 * - parseStateMd across YAML-frontmatter, body-fallback, and partial formats
 * - formatGtdState graceful degradation when fields are missing
 * - readGtdState walk-up search with proper bounds
 */
...
const { parseStateMd, formatGtdState, readGtdState } = require('../hooks/gtd-statusline.js');
...
describe('formatGtdState', () => {
  ...
  test('returns empty string for empty state', () => {
    assert.equal(formatGtdState({}), '');
  });

  test('returns only available parts when everything else is missing', () => {
    assert.equal(formatGtdState({ status: 'planning' }), 'planning');
  });
});

// ─── readGtdState ───────────────────────────────────────────────────────────

describe('readGtdState', () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gtd-statusline-test-'));
  ...
  test('returns null when no STATE.md exists in the walk-up chain', () => {
    ...
    assert.equal(readGtdState(nested), null);
  });
...
describe('context meter respects CLAUDE_CODE_AUTO_COMPACT_WINDOW (#2219)', () => {
  ...
  const hookPath = path.join(__dirname, '..', 'hooks', 'gtd-statusline.js');
  ...
  test('bridge used_pct is raw (CC-consistent) regardless of ACW setting (#2451)', () => {
    ...
    // This ensures gtd-context-monitor warning messages match CC native /context.
