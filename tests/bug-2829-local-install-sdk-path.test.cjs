/**
 * Regression test for #2829: `command not found: gtd-sdk` with local-mode install.
 *
 * Repro: a fresh `npx get-tasks-done@latest` install with the runtime set
 * to local mode left every `gtd-sdk query …` call site unable to resolve the
 ...
 * actually carries `sdk/dist/cli.js` and `bin/gtd-sdk.js` regardless of mode,
 ...
 * `gtd-sdk` is on PATH and self-link the shim into a user-writable PATH dir
 * if not, so `/gtd-plan` and friends can call `gtd-sdk query …` directly.
 *
...
describe('bug #2829: local-mode install must materialize gtd-sdk on PATH', () => {
  let tmpRoot;
  let sdkDir;
  let pathDir;
  let homeDir;
  ...
  beforeEach(() => {
    tmpRoot = createTempDir('gtd-2829-');
  ...
  test('isLocal=true with sdk/dist/cli.js present self-links gtd-sdk into an on-PATH HOME bin dir', () => {
    ...
    // The shim must be materialized so `gtd-sdk query …` resolves.
    const linkPath = path.join(localBin, 'gtd-sdk');
    assert.ok(
      fs.existsSync(linkPath),
      `local install must self-link gtd-sdk into ${linkPath}. Output:\n${combined}`,
    );
    // And the installer must report ready (matches the global-mode UX).
    assert.ok(
      /GTD SDK ready/.test(combined),
      `local install must print "GTD SDK ready" once gtd-sdk is on PATH. Output:\n${combined}`,
    );
  ...
  test('isLocal=true with dist present and no on-PATH HOME bin still warns rather than lying about readiness', () => {
    ...
    // probe still cannot find gtd-sdk on PATH.
    const { stdout, stderr } = captureConsole(() => {
      installSdkIfNeeded({ sdkDir, isLocal: true });
    });
    const combined = `${stdout}\n${stderr}`;
    assert.ok(
      !/GTD SDK ready/.test(combined),
      `installer must not falsely claim ready when gtd-sdk is not callable. Output:\n${combined}`,
    );
    assert.ok(
