/**
 * Tests for gtd-check-update-worker.js — Windows npm resolution platform gate.
 *
 ...
 * targets `.cjs` files in bin/lib/get-tasks-done — `hooks/*.js` is out of
...
describe('gtd-check-update-worker: Windows npm spawn platform gate', () => {
