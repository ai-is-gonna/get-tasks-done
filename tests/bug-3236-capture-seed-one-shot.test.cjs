...
const PLANT_SEED = path.join(ROOT, 'get-tasks-done', 'workflows', 'plant-seed.md');
...
function readPlantSeed() {
  try {
    ...
    throw new Error('get-tasks-done/workflows/plant-seed.md not found: ' + err.message);
  }
}
...
describe('#3236: plant-seed.md one-shot capture contract', () => {
  test('plant-seed.md exists', () => {
    assert.ok(
      ...
      'get-tasks-done/workflows/plant-seed.md does not exist',
    );
  });
