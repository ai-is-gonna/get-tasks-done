...
# gtd-graphify-update.sh — PostToolUse hook (Bash matcher) that auto-rebuilds
...
#   - Detaches hooks/lib/gtd-graphify-rebuild.sh which copies graphify-out/* to
...
node -e '
  ...
  const status = {
    ts: process.env.GTD_TS,
    status: "running",
    exit_code: null,
    duration_ms: null,
    head_at_build: process.env.GTD_HEAD,
...
REBUILD_SCRIPT="$HOOK_DIR/lib/gtd-graphify-rebuild.sh"
