#!/usr/bin/env bash
# gtd-graphify-rebuild.sh — detached rebuild runner for hooks/gtd-graphify-update.sh.
#
# Usage:
#   gtd-graphify-rebuild.sh <STATUS_FILE> <LOCK_FILE> <HEAD_SHA> <MS_START> <GRAPHIFY_BIN>
#
...
GTD_STATUS_TS="$TS_END" \
GTD_STATUS_NAME="$STATUS_NAME" \
GTD_EXIT_CODE="$EXIT_CODE" \
GTD_DURATION="$DURATION" \
GTD_HEAD_SHA="$HEAD_SHA" \
GTD_STATUS_FILE="$STATUS_FILE" \
node -e '
  const fs = require("node:fs");
  const status = {
    ts: process.env.GTD_STATUS_TS,
    status: process.env.GTD_STATUS_NAME,
    exit_code: parseInt(process.env.GTD_EXIT_CODE, 10),
    duration_ms: parseInt(process.env.GTD_DURATION, 10),
    head_at_build: process.env.GTD_HEAD_SHA,
    graphify_version: null,
  };
  fs.writeFileSync(process.env.GTD_STATUS_FILE, JSON.stringify(status, null, 2) + "\n");
' 2>/dev/null || true
