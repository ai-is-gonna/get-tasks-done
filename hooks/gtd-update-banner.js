...
// SessionStart banner that surfaces GTD update availability when GTD's
// statusline isn't installed. Reads the cache that
// gtd-check-update-worker.js writes to ~/.cache/gtd/gtd-update-check.json.
//
// Opt-in by design: bin/install.js only registers this hook when the user
// declines to install (or replace) the GTD statusline. The presence of the
...
function buildBannerOutput(state) {
  ...
  if (parseError) {
    ...
    return { systemMessage: 'GTD update check failed.' };
  }
  ...
  return {
    systemMessage: `GTD update available: ${installed} → ${latest}. Run /gtd:update.`,
  };
}
...
function main() {
  const cacheDir = path.join(os.homedir(), '.cache', 'gtd');
  ...
  if (parseError && !suppressFailureWarning) {
    ...
    // where ~/.cache/gtd was created by check-update but the parent dir got
    // wiped between runs.
    try {
