...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Quick sentinel check: skip config read entirely for non-GTD projects (#P2.5).
    ...
    // Detect if GTD is active (has .planning/STATE.md in working directory)
    const isGtdActive = fs.existsSync(path.join(cwd, '.planning', 'STATE.md'));

    // On CRITICAL with active GTD project, auto-record session state as a
    // breadcrumb for /gtd:resume-work (#1974). Fire-and-forget subprocess —
    ...
    if (isCritical && isGtdActive && !warnData.criticalRecorded) {
      try {
        // Runtime-agnostic path: this hook lives at <runtime-config>/hooks/
        // and gtd-tools.cjs lives at <runtime-config>/get-tasks-done/bin/.
        // Using __dirname makes this work on Claude Code, OpenCode, Gemini,
        // Kilo, etc. without hardcoding ~/.claude/.
        const gtdTools = path.join(__dirname, '..', 'get-tasks-done', 'bin', 'gtd-tools.cjs');
        ...
        spawn(
          process.execPath,
          [gtdTools, 'state', 'record-session', '--stopped-at', stoppedAt],
    ...
    if (isCritical) {
      message = isGtdActive
        ? `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          ...
          'GTD state is already tracked in STATE.md. Inform the user so they can run ' +
          '/gtd:pause-work at the next natural stopping point.'
        : `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
