...
// Shows: model | current task (or GTD state) | directory | context usage
...
function readLastSlashCommand(transcriptPath) {
  ...
  // Command names in Claude Code transcripts are plain identifiers like "gtd-plan-phase"
 ...
 *   - nextAction   : recommended next command ("work-task-issue") when idle, null otherwise
 ...
 *
 * All new fields default to undefined when absent — formatGtdState() degrades
...
function parseStateMd(content) {
  ...
  if (fmMatch) {
    ...
    for (const line of fm.split('\n')) {
      ...
      // next_action: recommended command when idle (discuss-phase / plan-phase / work-task-issue / verify-phase)
...
/**
 * Format GTD state into display string.
 *
 ...
 *   active_phase null + next_action set    → "v2.0 [██░] X% · next work-task-issue 4.5"
...
function runStatusline() {
  ...
  try {
    ...
    // GTD state (milestone · status · phase) — shown when no todo task
    const gtdStateStr = task ? '' : formatGtdState(readGtdState(dir) || {});

    // GTD update available?
    // Check shared cache first (#1421), fall back to runtime-specific cache for
    // backward compatibility with older gtd-check-update.js versions.
    let gtdUpdate = '';
    const sharedCacheFile = path.join(homeDir, '.cache', 'gtd', 'gtd-update-check.json');
    const legacyCacheFile = path.join(claudeDir, 'cache', 'gtd-update-check.json');
    const cacheFile = fs.existsSync(sharedCacheFile) ? sharedCacheFile : legacyCacheFile;
    if (fs.existsSync(cacheFile)) {
      try {
        const cache = JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
        if (cache.update_available) {
          gtdUpdate = '\x1b[33m⬆ /gtd:update\x1b[0m │ ';
        }
        if (cache.stale_hooks && cache.stale_hooks.length > 0) {
          // If installed version is ahead of npm latest, this is a dev install.
          // Running /gtd:update would downgrade — show a contextual warning instead.
          const isDevInstall = (() => {
          ...
          if (isDevInstall) {
            gtdUpdate += '\x1b[33m⚠ dev install — re-run installer to sync hooks\x1b[0m │ ';
          } else {
            gtdUpdate += '\x1b[31m⚠ stale hooks — run /gtd:update\x1b[0m │ ';
          }
        }
      } catch (e) {}
    }
    ...
    const middle = task
      ? `\x1b[1m${task}\x1b[0m`
      : gtdStateStr
        ? `\x1b[2m${gtdStateStr}\x1b[0m`
        : null;

    process.stdout.write(composeStatusline({ gtdUpdate, model, ctx, middle, dirname, lastCmdSuffix, position }));
  } catch (e) {
 ...
 *
 * @param {object} opts
 * @param {string} [opts.gtdUpdate='']      - leading update/stale-hooks warning (already formatted)
 ...
 * @param {string|null} [opts.middle=null]  - middle segment (todo task or GTD state), null = absent
...
function composeStatusline({
  ...
  if (pos === 'front') {
    if (middle) return `${gtdUpdate}${modelSeg}${ctx} │ ${middle} │ ${dirSeg}${lastCmdSuffix}`;
    return `${gtdUpdate}${modelSeg}${ctx} │ ${dirSeg}${lastCmdSuffix}`;
  }
  // 'end' — preserved byte-for-byte relative to original inline templates
  if (middle) return `${gtdUpdate}${modelSeg} │ ${middle} │ ${dirSeg}${ctx}${lastCmdSuffix}`;
  return `${gtdUpdate}${modelSeg} │ ${dirSeg}${ctx}${lastCmdSuffix}`;
}

// Export helpers for unit tests. Harmless when run as a script.
module.exports = {
  readGtdState, parseStateMd, formatGtdState,
  readGtdConfig, getConfigValue, readLastSlashCommand,
  composeStatusline,
};
...
function renderStatusline(data) {
  ...
  const gtdStateStr = formatGtdState(readGtdState(dir) || {});
  const middle = gtdStateStr ? `\x1b[2m${gtdStateStr}\x1b[0m` : null;
