...
describe('query tools error factory', () => {
  ...
  it('maps GTDError to failure with semantic exit code', () => {
    const err = toToolsErrorFromUnknown('state', ['load'], new GTDError('bad', ErrorClassification.Validation));
  ...
  it('maps timeout-like unknown errors to timeout classification', () => {
    const err = toToolsErrorFromUnknown('state', ['load'], new Error('gtd-tools timed out after 50ms: state load'));
