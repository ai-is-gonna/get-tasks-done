 ...
 *
 * Uses the same `loadConfig(cwd)` as `get-tasks-done/bin/lib/state.cjs` by resolving
 * `core.cjs` next to a shipped/bundled/user `get-tasks-done` install (same probe order
 * as `resolveGtdToolsPath`). This keeps JSON output **byte-compatible** with
 * `node gtd-tools.cjs state load` for monorepo and standard installs.
 *
 ...
 *
 * Port of `cmdStateLoad` from `get-tasks-done/bin/lib/state.cjs` lines 44–86.
 */
