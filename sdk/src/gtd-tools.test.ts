...
import { GTDTools, GTDToolsError, resolveGtdToolsPath } from './gtd-tools.js';
...
const BUNDLED_GTD_TOOLS_PATH = fileURLToPath(
...
describe('GTDTools', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `gtd-tools-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('exec()', () => {
    it('parses valid JSON output', async () => {
      ...
      const tools = new GTDTools({ projectDir: tmpDir, gtdToolsPath: scriptPath, preferNativeQuery: false });
    ...
    it('throws GTDToolsError on non-zero exit code', async () => {
      const scriptPath = await createScript(
        'fail.cjs',
      ...
      try {
        ...
        expect(err).toBeInstanceOf(GTDToolsError);
        const gtdErr = err as GTDToolsError;
        expect(gtdErr.command).toBe('state');
        expect(gtdErr.args).toEqual(['load']);
        expect(gtdErr.stderr).toContain('something went wrong');
        expect(gtdErr.exitCode).toBeGreaterThan(0);
      }
    });

    it('throws GTDToolsError with context when gtd-tools.cjs not found', async () => {
      const tools = new GTDTools({
        projectDir: tmpDir,
        gtdToolsPath: '/nonexistent/path/gtd-tools.cjs',
        preferNativeQuery: false,
      });

      await expect(tools.exec('state', ['load'])).rejects.toThrow(GTDToolsError);
    });
    ...
    it('throws parse error when stdout is non-JSON', async () => {
      ...
      try {
        ...
        expect(err).toBeInstanceOf(GTDToolsError);
        const gtdErr = err as GTDToolsError;
        expect(gtdErr.message).toContain('Failed to parse');
        expect(gtdErr.message).toContain('Not JSON at all');
      }
    });
    ...
    it('handles timeout by killing child process', async () => {
      ...
      try {
        ...
        expect(err).toBeInstanceOf(GTDToolsError);
        const gtdErr = err as GTDToolsError;
        expect(gtdErr.message).toContain('timed out');
      }
    }, 10_000);
    ...
    it('preserves GTDToolsError contract when native handler throws and fallback disabled', async () => {
      const scriptPath = await createScript(
        'should-not-run.cjs',
        `process.stdout.write(JSON.stringify({ should: 'not-run' }));`,
      );
      ...
      try {
        ...
        expect(err).toBeInstanceOf(GTDToolsError);
        const gtdErr = err as GTDToolsError;
        expect(gtdErr.command).toBe('verify.path-exists');
        expect(gtdErr.args).toEqual([]);
        expect(gtdErr.stderr).toBe('');
        expect(typeof gtdErr.exitCode === 'number').toBe(true);
      }
    });
  });
  ...
  describe('initNewProject()', () => {
    ...
    it('propagates errors from gtd-tools', async () => {
      const scriptPath = await createScript(
        'init-fail.cjs',
      ...
      await expect(tools.initNewProject()).rejects.toThrow(GTDToolsError);
    });
  });

  // ─── resolveGtdToolsPath() tests ────────────────────────────────────────

  describe('resolveGtdToolsPath()', () => {
    it('prefers bundled gtd-tools over project .claude when the bundled file exists', async () => {
      const localBinDir = join(tmpDir, '.claude', 'get-tasks-done', 'bin');
      await mkdir(localBinDir, { recursive: true });
      await writeFile(join(localBinDir, 'gtd-tools.cjs'), '// stub');

      const result = resolveGtdToolsPath(tmpDir);
      if (existsSync(BUNDLED_GTD_TOOLS_PATH)) {
        expect(result).toBe(BUNDLED_GTD_TOOLS_PATH);
      } else {
        expect(result).toBe(join(localBinDir, 'gtd-tools.cjs'));
      }
    });

    it('falls back to bundled repo path when repo-local does not exist', () => {
      const result = resolveGtdToolsPath(tmpDir);
      const expected = existsSync(BUNDLED_GTD_TOOLS_PATH)
    ...
    it('uses explicit gtdToolsPath when provided (overrides bundled / .claude resolution)', async () => {
      const localBinDir = join(tmpDir, '.claude', 'get-tasks-done', 'bin');
      await mkdir(localBinDir, { recursive: true });
      const scriptPath = join(localBinDir, 'gtd-tools.cjs');
      await writeFile(
        scriptPath,
