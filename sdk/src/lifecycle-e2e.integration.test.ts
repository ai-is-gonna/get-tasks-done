/**
 * E2E lifecycle integration test — proves GTD.runPhase() drives
 * the full phase lifecycle: discuss → research → plan → execute → verify → advance
 * after bootstrapping a real project via InitRunner.
 *
 * This is the capstone proof that `gtd-sdk auto` works end-to-end
 * without human intervention. InitRunner bootstraps the project,
 * then GTD.runPhase() drives Phase 1 through the complete lifecycle.
 *
...
import type { GTDEvent, PhaseRunnerResult, RoadmapAnalysis } from './types.js';
...
describe.skipIf(!cliAvailable || !gtdToolsAvailable)('E2E Lifecycle: InitRunner → GTD.runPhase() full lifecycle', () => {
  let tmpDir: string;
  ...
  beforeAll(async () => {
    tmpDir = await mkdtemp(join(tmpdir(), 'gtd-sdk-lifecycle-e2e-'));
    ...
    const initEventStream = new GTDEventStream();
  ...
  it('GTD.runPhase() drives Phase 1 through the full lifecycle without human intervention', async () => {
    ...
    // Construct GTD with autoMode: true
    const gtd = new GTD({
      projectDir: tmpDir,
      autoMode: true,
    });
    gtd.onEvent((e: GTDEvent) => events.push(e));

    // Run the discovered first phase with tight budget to minimize cost
    const result: PhaseRunnerResult = await gtd.runPhase(phase1Number!, {
      maxTurnsPerStep: 10,
    ...
    if (phaseComplete.type === GTDEventType.PhaseComplete) {
    ...
    const stepStartEvents = events.filter(
      (e): e is Extract<GTDEvent, { type: GTDEventType.PhaseStepStart }> =>
        e.type === GTDEventType.PhaseStepStart,
    );
    ...
    const stepCompleteEvents = events.filter(
      (e): e is Extract<GTDEvent, { type: GTDEventType.PhaseStepComplete }> =>
        e.type === GTDEventType.PhaseStepComplete,
    );
