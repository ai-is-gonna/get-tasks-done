...
function stripGitSuffix(value: unknown): string {
  return String(value || '').replace(/\.git$/i, '');
}

function repoFromParts(owner: string, name: string): string | null {
  return normalizeGitHubRepo(`${owner || ''}/${stripGitSuffix(name || '')}`);
}

export function parseGitHubRemoteUrl(remoteUrl: unknown): string | null {
...
function gitRemoteUrls(cwd: string, remote: string): string[] {
  const urls: string[] = [];
  for (const args of [
...
export function resolveGitHubRepoFromGit(cwd: string): Record<string, unknown> {
  const root = runGit(cwd, ['rev-parse', '--show-toplevel']);
  if (!root.ok || !root.stdout) {
    return { ok: false, reason: 'not_git_repo', message: 'not inside a git repository' };
  }
  ...
  const remotes = remotesResult.ok
    ...
    : [];
  if (remotes.length === 0) {
    return { ok: false, reason: 'no_git_remotes', git_root: root.stdout, message: 'no git remotes are configured' };
  }

  const candidates: Array<{ remote: string; url: string; repo: string }> = [];
  ...
  if (candidates.length === 0) {
    return { ok: false, reason: 'no_github_remote', git_root: root.stdout, message: 'no GitHub remote is configured' };
  }
  ...
  if (originCandidates.length > 0) {
    ...
    if (originRepos.length === 1) {
      return { ok: true, repo: originRepos[0], source: 'git_remote:origin', remote: 'origin', git_root: root.stdout };
    }
    return {
      ok: false,
  ...
  if (repos.length === 1) {
    ...
    return { ok: true, repo: repos[0], source: `git_remote:${candidate.remote}`, remote: candidate.remote, git_root: root.stdout };
  }
  ...
  return {
    ...
    git_root: root.stdout,
    message: `multiple GitHub remotes are configured: ${candidates.map((candidate) => `${candidate.repo} (${candidate.remote})`).join(', ')}`,
  };
}
