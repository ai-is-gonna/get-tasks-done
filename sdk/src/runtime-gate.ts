 ...
 *
 * The autonomous SDK orchestrator (`gtd-sdk auto`) currently drives plan
 ...
 * up front and points users at the supported in-session GTD slash commands
 ...
 *
 * Precedence mirrors `detectRuntime`: `GTD_RUNTIME` env var > `config.runtime`
...
export function assertRuntimeSupportsAutoMode(config?: Record<string, unknown> | { runtime?: unknown }): void {
  ...
  // a `GTD_RUNTIME` value that isn't in SUPPORTED_RUNTIMES falls through to
  // the config tier, so reporting it as the source would be misleading.
  const env = process.env.GTD_RUNTIME;
  ...
  throw new Error(
    `gtd-sdk auto currently supports the Claude runtime only ` +
    `(detected runtime=${runtime} via ${source}). ` +
    `Autonomous terminal runs through the Claude Agent SDK; non-Claude ` +
    `runtimes (Codex, Gemini, OpenCode, etc.) must drive GTD via the ` +
    `in-session slash commands (e.g. /gtd-discuss-phase, /gtd-plan-phase, ` +
    `/gtd-export-phase-issues, /gtd-work-task-issue) until issue #2832 lands a multi-runtime task runner. ` +
    `To run on Claude anyway, unset GTD_RUNTIME and set runtime: "claude" ` +
