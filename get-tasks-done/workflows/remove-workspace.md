<purpose>
Remove a GTD workspace, cleaning up git worktrees and deleting the workspace directory.
</purpose>
...
```bash
INIT=$(gtd-sdk query init.remove-workspace "$WORKSPACE_NAME")
...
First run `/gtd-list-workspaces` to show available workspaces, then ask:
