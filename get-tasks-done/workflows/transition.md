...
There is no `/gtd-transition` command. This workflow is invoked automatically by
task issue orchestration during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/gtd-transition`.

**Valid user commands for phase progression:**
- `/gtd:discuss-phase {N}` — discuss a phase before planning
- `/gtd:plan-phase {N}` — plan a phase
- `/gtd:work-task-issue --phase {N}` — work exported task issues
- `/gtd:progress` — see roadmap progress
...
These will carry forward as debt. Review: `/gtd:audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to `gtd-sdk query phase.complete`:**

```bash
TRANSITION=$(gtd-sdk query phase.complete "${current_phase}")
```
...
```text
@~/.claude/get-tasks-done/workflows/graduation.md
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `gtd-sdk query phase.complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(gtd-sdk query progress.bar --raw)
```
...
**Use the transition result from `gtd-sdk query phase.complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `GTD_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$GTD_WORKSTREAM" ]; then
  WS_LIST=$(gtd-sdk query workstream.list --raw)
fi
```
...
Filter out the current workstream (`$GTD_WORKSTREAM`) and any workstreams with
...
Exit skill and invoke SlashCommand("/gtd:plan-phase [X+1] --auto ${GTD_WS}")
...
Exit skill and invoke SlashCommand("/gtd:discuss-phase [X+1] --auto ${GTD_WS}")
...
`/gtd:discuss-phase [X+1] ${GTD_WS}` — gather context and clarify approach

---

**Also available:**
- `/gtd:plan-phase [X+1] ${GTD_WS}` — skip discussion, plan directly
- `/gtd:plan-phase --research-phase [X+1] ${GTD_WS}` — investigate unknowns
...
`/gtd:plan-phase [X+1] ${GTD_WS}`

---

**Also available:**
- `/gtd:discuss-phase [X+1] ${GTD_WS}` — revisit context
- `/gtd:plan-phase --research-phase [X+1] ${GTD_WS}` — investigate unknowns
...
```bash
gtd-sdk query config-set workflow._auto_chain_active false
```
...
`/gtd:workstreams complete {current_ws_name} ${GTD_WS}`

See overall milestone progress:

`/gtd:workstreams progress ${GTD_WS}`
...
Do NOT suggest `/gtd:complete-milestone` or `/gtd:new-milestone`.
...
Exit skill and invoke SlashCommand("/gtd:complete-milestone {version} ${GTD_WS}")
...
`/gtd:complete-milestone {version} ${GTD_WS}`
