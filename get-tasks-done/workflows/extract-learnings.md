...
```bash
gtd-sdk query state.update "Last Activity" "$(date +%Y-%m-%d)"
```
</step>
...
- /gtd:progress — see overall project state
- /gtd:work-task-issue --phase {next} — continue to next phase
