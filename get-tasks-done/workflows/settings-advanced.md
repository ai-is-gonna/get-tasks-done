<purpose>
Interactive configuration of GTD power-user knobs — plan bounce, node repair, subagent timeouts,
inline plan threshold, base branch, branch templates, response language,
context window, gitignored search, graphify build timeout, and runtime model tier overrides.

This is a companion to `/gtd:settings` — the common-case prompt there covers model profile,
...
All subsequent reads and writes go through `$GTD_CONFIG_PATH`. Never hardcode
...
- `git.phase_branch_template` (default: `gtd/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `gtd/{milestone}-{slug}`)
...
**Null-allowed fields.** For `response_language` and `workflow.plan_bounce_script`, an empty
input clears the field (`null`). A non-empty input is stored verbatim as a string.
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or gtd/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or gtd/{milestone}-{slug}>)",
...
This section lets the user inspect and override the built-in model IDs GTD resolves for each
...
`gtd-sdk query config-set runtime <value>` before proceeding to Step C.
...
```bash
gtd-sdk query config-set model_profile_overrides.<runtime>.<tier> "<model-id>"
```

For each tier where the user chose "Clear override", remove the key by setting it to null:
```bash
gtd-sdk query config-set model_profile_overrides.<runtime>.<tier> null
```
...
Merge the new settings into the existing config at `$GTD_CONFIG_PATH`. This merge is the
core correctness invariant: **preserve every unrelated key** — do not clobber siblings.

Apply each selected value via `gtd-sdk query config-set <key> <value>` so the central
...
gtd-sdk query config-set workflow.plan_bounce_passes 5
gtd-sdk query config-set workflow.subagent_timeout 900
gtd-sdk query config-set git.base_branch main
gtd-sdk query config-set context_window 1000000
# Runtime model tier examples:
gtd-sdk query config-set runtime gemini
gtd-sdk query config-set model_profile_overrides.gemini.opus gemini-3-ultra
gtd-sdk query config-set model_profile_overrides.gemini.haiku null
```
...
route each write through `gtd-sdk query config-set` so sibling preservation is handled by
the central setter.
</step>
...
These settings apply to future /gtd:plan-phase, task issue, and
/gtd:discuss-phase runs.

For common-case toggles (model profile, research/plan_check/verifier, branching strategy,
UI/AI phase gates), use /gtd:settings.
```
</step>
...
<success_criteria>
- [ ] Current config read from resolved `$GTD_CONFIG_PATH`
- [ ] Six sections rendered (Planning, Execution, Discussion, Git, Runtime/Output, Runtime Model Tiers)
...
- [ ] Writes routed through `gtd-sdk query config-set` so unrelated keys are preserved
