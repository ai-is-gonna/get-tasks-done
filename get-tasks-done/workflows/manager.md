...
```bash
INIT=$(gtd-sdk query init.manager)
...
- `manager_flags.discuss` — appended to `/gtd:discuss-phase` args (e.g. `"--auto --analyze"`)
- `manager_flags.plan` — appended to plan agent init command
- `manager_flags.execute` — appended to execute agent init command

These are empty strings by default. Set via: `gtd-sdk query config-set manager.flags.discuss "--auto --analyze"`
...
All {phase_count} phases done. Ready for final steps:
  → /gtd:verify-work — run acceptance testing
  → /gtd:complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="gtd-verify-work")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="gtd-complete-milestone")` then exit.
...
**Auto-refresh:** If background agents are running (`is_active` is true for any phase), set a 60-second auto-refresh cycle. After presenting the action menu, if no user input is received within 60 seconds, automatically refresh the dashboard. This interval is configurable via `manager_refresh_interval` in GTD config (default: 60 seconds, set to 0 to disable).
...
```
Skill(skill="gtd-discuss-phase", args="{PHASE_NUM} {manager_flags.discuss}")
```
...
Agent(
  ...
  prompt="You are running the GTD plan-phase workflow for phase {N} of the project.
...
Skill(skill=\"gtd-plan-phase\", args=\"{N} --auto {manager_flags.plan}\")
...
### Work Phase N Through Task Issues

Task work runs autonomously. Spawn a background agent that delegates to the task issue Skill pipeline with any configured flags:

```
Agent(
  description="Work phase {N} tasks: {phase_name}",
  run_in_background=true,
  prompt="You are running the GTD task issue workflow for phase {N} of the project.
...
Run the task issue workflow with any configured manager flags:
Skill(skill=\"gtd-export-phase-issues\", args=\"{N} {manager_flags.export}\")
Skill(skill=\"gtd-work-task-issue\", args=\"--phase {N} {manager_flags.execute}\")

This delegates implementation to exported task issues, isolated task branches, task PRs, reconciliation, and final phase gates.
...
- Display the error clearly, then offer to fix it:
  ...
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill — use `Skill(skill="gtd-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="gtd-work-task-issue", args="--phase {N}")` if the failed action was task work. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill — use `Skill(skill="gtd-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="gtd-work-task-issue", args="--phase {N}")` if the failed action was task work. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /gtd:manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/gtd:manager` or `/gtd:progress` invocation.
