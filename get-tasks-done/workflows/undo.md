<purpose>
Safe git revert workflow. Rolls back GTD phase or plan commits using the phase manifest with dependency checks and a confirmation gate. Uses git revert --no-commit (NEVER git reset) to preserve history.
</purpose>
...
```
Usage: /gtd:undo --last N | --phase NN | --plan NN-MM

Modes:
  --last N      Show last N GTD commits for interactive selection
...
Filter for GTD conventional commits matching `type(scope): message` pattern (e.g., `feat(04-01):`, `docs(03):`, `fix(02-03):`).
...
```
Working tree has uncommitted changes. Commit or stash them before running /gtd:undo.
```
...
**Also available:**
- `/gtd:work-task-issue --phase ${PHASE}` — re-execute if needed
- `/gtd:undo --last 1` — undo the revert itself if something went wrong
