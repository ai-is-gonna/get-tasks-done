...
```bash
AUDIT=$(gtd-sdk query audit-uat --raw)
```
...
1. **Close stale items:** `/gtd:verify-work {phase}` — mark stale tests as resolved
2. **Run active tests:** Human UAT test plan below
3. **When prerequisites met:** Retest blocked items with `/gtd:verify-work {phase}`
```
</step>
