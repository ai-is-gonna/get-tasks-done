...
gtd-plan-phase N → gtd-review N --codex → gtd-plan-phase N --reviews → gtd-review N --codex → ...
...
@$HOME/.claude/get-tasks-done/references/revision-loop.md
@$HOME/.claude/get-tasks-done/references/gates.md
@$HOME/.claude/get-tasks-done/references/agent-contracts.md
</required_reading>
...
GTD_WS=""
echo "$ARGUMENTS" | grep -qE '\-\-ws\s+\S+' && GTD_WS=$(echo "$ARGUMENTS" | grep -oE '\-\-ws\s+\S+')
```

## 1.5. Config Gate (feature disabled by default)

```bash
CONVERGENCE_ENABLED=$(gtd-sdk query config-get workflow.plan_review_convergence 2>/dev/null || echo "false")
```

**If `CONVERGENCE_ENABLED` is not `"true"`:** Display and exit:

```text
gtd-plan-review-convergence is disabled (workflow.plan_review_convergence=false).
...
Then re-run: /gtd:plan-review-convergence {PHASE}
```

## 2. Initialize

```bash
INIT=$(node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► PLAN CONVERGENCE — Phase {phase_number}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Agent(
  ...
  prompt="Run /gtd:plan-phase for Phase {PHASE}.

Execute: Skill(skill='gtd-plan-phase', args='{PHASE} {GTD_WS}')
...
Agent(
  ...
  prompt="Run /gtd:review for Phase {PHASE}.

Execute: Skill(skill='gtd-review', args='--phase {PHASE} {REVIEWER_FLAGS} {GTD_WS}')
...
```bash
node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```

Display:
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► CONVERGENCE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Next: /gtd:work-task-issue --phase {PHASE}
```
...
To replan manually:  /gtd:plan-phase {PHASE} --reviews
To restart loop:     /gtd:plan-review-convergence {PHASE} {REVIEWER_FLAGS}
```
Exit workflow.
...
Agent(
  ...
  prompt="Run /gtd:plan-phase with --reviews for Phase {PHASE}.

Execute: Skill(skill='gtd-plan-phase', args='{PHASE} --reviews --skip-research {GTD_WS}')
...
IMPORTANT: When gtd-plan-phase outputs '## PLANNING COMPLETE', that means replanning is done. Return at that point.",
  mode="auto"
)
```
...
- [ ] Initial planning via Agent → Skill("gtd-plan-phase") if no plans exist
- [ ] Review via Agent → Skill("gtd-review") — isolated, not inline; {GTD_WS} forwarded
- [ ] Replan via Agent → Skill("gtd-plan-phase --reviews") — isolated, not inline
