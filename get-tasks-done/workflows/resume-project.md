...
<required_reading>
@~/.claude/get-tasks-done/references/continuation-format.md
</required_reading>
...
```bash
INIT=$(gtd-sdk query init.resume)
...
**If `planning_exists` is false:** This is a new project - route to /gtd:new-project
</step>
...
- This is the primary resumption source — structured data from `/gtd:pause-work`
...
📋 [N] pending todos — /gtd:capture --list to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/gtd:work-task-issue --phase {phase} ${GTD_WS})
   OR
1. Discuss Phase 3 context (/gtd:discuss-phase 3 ${GTD_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/gtd:plan-phase 3 ${GTD_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Execute plan** → Show direct next command:
  ...
  **{phase}-{plan}: [Plan Name]** — [objective from PLAN.md]
  ...
  `/gtd:work-task-issue --phase {phase} ${GTD_WS}`
...
- **Plan phase** → Show direct next command:
  ...
  **Also available:**
  - `/gtd:discuss-phase [N] ${GTD_WS}` — gather context first
  - `/gtd:plan-phase --research-phase [N] ${GTD_WS}` — investigate unknowns
