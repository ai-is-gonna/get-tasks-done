...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/gtd-note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
{count} active note(s). Use `/gtd-note promote <N>` to convert to a todo.
```
...
4. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a GTD project. Run `/gtd:new-project` to initialize one."
...
<edge_cases>
1. **"list" as note text**: `/gtd-note list of things` saves note "list of things" (subcommand only when `list` is the entire arg)
2. **No `.planning/`**: Falls back to global `~/.claude/notes/` — works in any directory
3. **Promote without project**: Warns that todos require `.planning/`, suggests `/gtd:new-project`
