...
> `$HOME/.claude/get-tasks-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
ADVISOR_MODEL=$(gtd-sdk query resolve-model gtd-advisor-researcher --raw)
```
...
```bash
PROFILE_CONTENT=$(cat "$HOME/.claude/get-tasks-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @~/.claude/agents/gtd-advisor-researcher.md for your role and instructions.
