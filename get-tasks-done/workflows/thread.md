...
Invoked by `/gtd:thread` (`commands/gtd/thread.md`).
...
- Read frontmatter `status` field via:
  ```bash
  gtd-sdk query frontmatter.get .planning/threads/{file} status
  ```
...
```
No threads found. Create one with: /gtd:thread <description>
```
...
2. Update the thread file's frontmatter `status` field to `resolved` and `updated` to today's ISO date:
   ```bash
   gtd-sdk query frontmatter.set .planning/threads/{SLUG}.md status resolved
   gtd-sdk query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
   ```

3. Commit:
   ```bash
   gtd-sdk query commit "docs: resolve thread — {SLUG}" --files ".planning/threads/{SLUG}.md"
   ```
...
2. Read the file and display a summary:
   ...
   ─────────────────────────────────────
   Resume with: /gtd:thread {SLUG}
   Close with:  /gtd:thread close {SLUG}
   ```
...
```bash
gtd-sdk query frontmatter.set .planning/threads/{SLUG}.md status in_progress
gtd-sdk query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
```
...
1. Generate slug from description:
   ```bash
   SLUG=$(gtd-sdk query generate-slug "$ARGUMENTS" --raw)
   ```
...
5. Commit:
   ```bash
   gtd-sdk query commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
...
6. Report:
   ...
   Resume anytime with: /gtd:thread {slug}
   Close when done with: /gtd:thread close {slug}
   ```
</mode_create>
...
- Lighter weight than /gtd:pause-work — no phase state, no plan context
- The value is in Context and Next Steps — a cold-start session can pick up immediately
- Threads can be promoted to phases or backlog items when they mature:
  /gtd-add-phase or /gtd-add-backlog with context from the thread
- Thread files live in .planning/threads/ — no collision with phases or other GTD structures
...
- Status fields read via gtd-sdk query frontmatter.get — never eval'd or shell-expanded
- The generate-slug call for new threads runs through gtd-sdk query (or gtd-tools) which sanitizes input — keep that pattern
</security_notes>
