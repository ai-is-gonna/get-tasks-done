...
<required_reading>
@~/.claude/get-tasks-done/references/domain-probes.md
@~/.claude/get-tasks-done/references/gate-prompts.md
@~/.claude/get-tasks-done/references/universal-anti-patterns.md
</required_reading>
...
1. **gtd-phase-researcher** — Reads CONTEXT.md to know WHAT to research
2. **gtd-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/gtd:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.

<step name="initialize" priority="first">
Phase number from argument (required).

```bash
INIT=$(gtd-sdk query init.phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(gtd-sdk query agent-skills gtd-advisor-researcher)
```
...
Use /gtd:progress ${GTD_WS} to see available phases.
```
Exit workflow.
...
if [ -f "$HOME/.claude/get-tasks-done/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Check if a SPEC.md (from `/gtd:spec-phase`) exists for this phase. SPEC.md locks requirements before implementation decisions.
...
**Note:** SPEC.md files named `AI-SPEC.md` (from `/gtd:ai-integration-phase`) are excluded — different purpose.
</step>
...
If raw spikes/sketches exist but no findings skill, note: `⚠ Unpackaged spikes/sketches detected — run /gtd:spike --wrap-up or /gtd:sketch --wrap-up to make findings available.`
...
```bash
TODO_MATCHES=$(gtd-sdk query todo.match-phase "${PHASE_NUMBER}")
```
...
Read `@~/.claude/get-tasks-done/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
`/gtd:plan-phase ${PHASE} ${GTD_WS}`

---

**Also available:** `--chain` for auto plan+execute after; `/gtd:plan-phase ${PHASE} --skip-research ${GTD_WS}` to plan without research; `/gtd:ui-phase ${PHASE} ${GTD_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
...
```bash
gtd-sdk query commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
gtd-sdk query state.record-session \
  --stopped-at "Phase ${PHASE} context gathered" \
  --resume-file "${phase_dir}/${padded_phase}-CONTEXT.md"

gtd-sdk query commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
