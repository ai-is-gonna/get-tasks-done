<purpose>
Generate an AI design contract (AI-SPEC.md) for phases that involve building AI systems. Orchestrates gtd-framework-selector → gtd-ai-researcher → gtd-domain-researcher → gtd-eval-planner with a validation gate. Inserts between discuss-phase and plan-phase in the GTD lifecycle.
...
<required_reading>
@~/.claude/get-tasks-done/references/ai-frameworks.md
@~/.claude/get-tasks-done/references/ai-evals.md
</required_reading>
...
```bash
SELECTOR_MODEL=$(gtd-sdk query resolve-model gtd-framework-selector 2>/dev/null | jq -r '.model' 2>/dev/null || true)
RESEARCHER_MODEL=$(gtd-sdk query resolve-model gtd-ai-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
DOMAIN_MODEL=$(gtd-sdk query resolve-model gtd-domain-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
PLANNER_MODEL=$(gtd-sdk query resolve-model gtd-eval-planner 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Check config:
```bash
AI_PHASE_ENABLED=$(gtd-sdk query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```

**If `AI_PHASE_ENABLED` is `false`:**
```
AI phase is disabled in config. Enable via /gtd:settings.
```
Exit workflow.
...
Recommended: run /gtd:discuss-phase {N} first to capture framework preferences.
...
## 5. Spawn gtd-framework-selector

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► AI DESIGN CONTRACT — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Step 1/4 — Framework Selection...
```

Spawn `gtd-framework-selector` with:
```markdown
Read ~/.claude/agents/gtd-framework-selector.md for instructions.
...
**If selector fails or returns empty:** Exit with error — "Framework selection failed. Re-run /gtd:ai-integration-phase {N} or answer the framework question in /gtd:discuss-phase {N} first."

## 6. Initialize AI-SPEC.md

Copy template:
```bash
cp "$HOME/.claude/get-tasks-done/templates/AI-SPEC.md" "${PHASE_DIR}/${PADDED_PHASE}-AI-SPEC.md"
```
...
Spawn `gtd-ai-researcher` with:
```markdown
Read ~/.claude/agents/gtd-ai-researcher.md for instructions.
...
Spawn `gtd-domain-researcher` with:
```markdown
Read ~/.claude/agents/gtd-domain-researcher.md for instructions.
...
Spawn `gtd-eval-planner` with:
```markdown
Read ~/.claude/agents/gtd-eval-planner.md for instructions.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► AI-SPEC COMPLETE — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Next step:
  /gtd:plan-phase {N}   — planner will consume AI-SPEC.md
```
