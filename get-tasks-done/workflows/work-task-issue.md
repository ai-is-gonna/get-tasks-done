...
@~/.claude/get-tasks-done/references/worktree-path-safety.md
</required_reading>
...
<read_only_mode>
If `$ARGUMENTS` contains `--read-only`, run:

```bash
gtd-sdk query work-task-issue $ARGUMENTS
```
...
If the result is ready and `$ARGUMENTS` does not contain `--execute`, report the
preview and tell the user to run `work-task-issue --complete-phase <phase> --execute`.

If the result is ready and `$ARGUMENTS` contains `--execute`, run only the
...
Respect the code review workflow toggle before running phase-completion review:

```bash
CODE_REVIEW_ENABLED=$(gtd-sdk query config-get workflow.code_review 2>/dev/null || echo "true")
```

If the value is `"false"`, skip this gate with the message
`Code review skipped (workflow.code_review=false)` and continue to the
regression gate.

Otherwise run the standard code review workflow for the completed phase before
continuing to regression. If review produces blocking findings, stop and report
the review artifact and required fix command.
</step>
...
```bash
gtd-sdk query work-task-issue $ARGUMENTS --reconcile --read-only
```
...
```bash
gtd-sdk query work-task-issue $ARGUMENTS --reconcile --execute
```
...
```bash
gtd-sdk query work-task-issue $ARGUMENTS --execute
```
