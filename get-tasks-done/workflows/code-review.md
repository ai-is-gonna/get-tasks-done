<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns gtd-code-reviewer agent, commits REVIEW.md, and presents results to user.
</purpose>
...
- gtd-code-reviewer: Reviews source files for bugs and quality issues
</available_agent_types>
...
```
Error: Phase ${PHASE_ARG} not found. Run /gtd:progress to see available phases.
```
...
2. Config value: `gtd-sdk query config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(gtd-sdk query config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /gtd:code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
```bash
FALLOW_ENABLED=$(gtd-sdk query config-get code_quality.fallow.enabled 2>/dev/null || echo "false")
FALLOW_SCOPE=$(gtd-sdk query config-get code_quality.fallow.scope 2>/dev/null || echo "phase")
FALLOW_PROFILE=$(gtd-sdk query config-get code_quality.fallow.profile 2>/dev/null || echo "standard")
FALLOW_MCP=$(gtd-sdk query config-get code_quality.fallow.mcp 2>/dev/null || echo "false")
```
...
const { resolveFallowBinary } = require('./get-tasks-done/bin/lib/fallow-runner.cjs');
...
Spawn the gtd-code-reviewer agent:

```
Agent(subagent_type="gtd-code-reviewer", prompt="
<files_to_read>
${FILES_TO_READ}
</files_to_read>
...
No REVIEW.md created. You can retry with /gtd:code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with /gtd:code-review ${PHASE_ARG}"
fi
```
</step>
...
Next steps:
  /gtd:code-review ${PHASE_NUMBER} --fix  — Auto-fix issues
