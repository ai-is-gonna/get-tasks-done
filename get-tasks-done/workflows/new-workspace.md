...
```bash
INIT=$(gtd-sdk query init.new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /gtd:workspace --new --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /gtd:workspace --new --name my-workspace --repos repo1,repo2 --auto
```
Exit.
