...
- Check for MILESTONE-CONTEXT.md (from /gtd-discuss-milestone)
...
```bash
gtd-sdk query state.milestone-switch --milestone "v[X.Y]" --name "[Name]"
```
...
```bash
gtd-sdk query phases.clear --confirm
```

```bash
gtd-sdk query commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(gtd-sdk query init.new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/gtd:new-milestone --reset-phase-numbers ${GTD_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/gtd:plan-phase` behavior. To change the default, use `/gtd:settings`.
...
Spawn 4 parallel gtd-project-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: ~/.claude/get-tasks-done/templates/research-project/{FILE}
</output>
", subagent_type="gtd-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
gtd-sdk query commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
gtd-sdk query commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
```bash
gtd-sdk query commit "docs: tag [count] pending todos with resolves_phase after milestone v[X.Y] roadmap" --files .planning/todos/pending/*.md
```
...
`/gtd:discuss-phase [N] ${GTD_WS}` — gather context and clarify approach

Also: `/gtd:plan-phase [N] ${GTD_WS}` — skip discussion, plan directly
```
...
- [ ] gtd-roadmapper spawned with phase numbering context
...
- [ ] User knows next step: `/gtd:discuss-phase [N] ${GTD_WS}`
