...
Invoked by `/gtd:update --reapply` (`commands/gtd/update.md`).

After a GTD update wipes and reinstalls files, this workflow merges user's previously saved local modifications back into the new version. Uses three-way comparison (pristine baseline, user-modified backup, newly installed version) to reliably distinguish user customizations from version drift.

**Critical invariant:** Every file in `gtd-local-patches/` was backed up because the installer's hash comparison detected it was modified. The workflow must NEVER conclude "no custom content" for any backed-up file — that is a logical contradiction. When in doubt, classify as CONFLICT requiring user review, not SKIP.
...
if [ -n "$KILO_CONFIG_DIR" ]; then
  candidate="$(expand_home "$KILO_CONFIG_DIR")/gtd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$KILO_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$KILO_CONFIG")")/gtd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/kilo/gtd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$OPENCODE_CONFIG_DIR")/gtd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$OPENCODE_CONFIG")")/gtd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/opencode/gtd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$GEMINI_CONFIG_DIR" ]; then
  candidate="$(expand_home "$GEMINI_CONFIG_DIR")/gtd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CODEX_HOME" ]; then
  candidate="$(expand_home "$CODEX_HOME")/gtd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CLAUDE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$CLAUDE_CONFIG_DIR")/gtd-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/gtd-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/kilo/gtd-local-patches"
  elif [ -d "$HOME/.config/opencode/gtd-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/opencode/gtd-local-patches"
  elif [ -d "$HOME/.opencode/gtd-local-patches" ]; then
    PATCHES_DIR="$HOME/.opencode/gtd-local-patches"
  elif [ -d "$HOME/.gemini/gtd-local-patches" ]; then
    PATCHES_DIR="$HOME/.gemini/gtd-local-patches"
  elif [ -d "$HOME/.codex/gtd-local-patches" ]; then
    PATCHES_DIR="$HOME/.codex/gtd-local-patches"
  else
    PATCHES_DIR="$HOME/.claude/gtd-local-patches"
  fi
fi
# Local install fallback — check all runtime directories
if [ ! -d "$PATCHES_DIR" ]; then
  for dir in .config/kilo .kilo .config/opencode .opencode .gemini .codex .claude; do
    if [ -d "./$dir/gtd-local-patches" ]; then
      PATCHES_DIR="./$dir/gtd-local-patches"
      break
    fi
  done
fi
...
Local patches are automatically saved when you run /gtd:update
after modifying any GTD workflow, command, or agent files.
```
Exit.

## Step 2: Determine baseline for three-way comparison

The quality of the merge depends on having a **pristine baseline** — the original unmodified version of each file from the pre-update GTD release. This enables three-way comparison:
- **Pristine baseline** (original GTD file before any user edits)
- **User's version** (backed up in `gtd-local-patches/`)
...
**Why this matters:** `git log --diff-filter=A` returns the commit that *first added* the file, which is the wrong baseline on repos that have been through multiple GTD update cycles. The `pristine_hashes` field in `backup-meta.json` records the SHA-256 of the file as it existed in the pre-update GTD release — matching against it finds the correct baseline regardless of how many updates have occurred.

### Option B: Pristine snapshot directory
Check if a `gtd-pristine/` directory exists alongside `gtd-local-patches/`:
```bash
PRISTINE_DIR="$CONFIG_DIR/gtd-pristine"
```
...
1. **Read the backed-up version** (user's modified copy from `gtd-local-patches/`)
2. **Read the newly installed version** (current file after update)
3. **If available, read the pristine baseline** (from git history or `gtd-pristine/`)
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `$HOME/.claude/`), variable additions (`${GTD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
...
git -C "$CONFIG_DIR" log --oneline --no-merges -- "{file_path}" | grep -v "gtd:update\|gtd-update\|GTD update\|gtd-install"
```
...
Run the verifier as a child process (the gtd-tools binary directory is not required — the script ships under `get-tasks-done/bin/` in the source repo and is installed to `${GTD_HOME}/get-tasks-done/bin/`; it is also exposed via the SDK at `sdk/dist/cli.js verify-reapply` when present):
...
VERIFY_OUTPUT="$(node "${GTD_HOME}/get-tasks-done/bin/verify-reapply-patches.cjs" "${VERIFY_ARGS[@]}")"
VERIFY_STATUS=$?
```
...
Then re-run /gtd:update --reapply to re-verify.
```
...
/gtd:update --reapply to retry with full verification.
```
...
Ask user:
- "Keep patch backups for reference?" → preserve `gtd-local-patches/`
- "Clean up patch backups?" → remove `gtd-local-patches/` directory
...
- [ ] Three-way merge used when pristine baseline available (git history or gtd-pristine/)
