<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /gtd:plan-phase --gaps.
...
```bash
GTD_WS=""
echo "$ARGUMENTS" | grep -qE -- '--ws[[:space:]]+[^[:space:]]+' && GTD_WS=$(echo "$ARGUMENTS" | grep -oE -- '--ws[[:space:]]+[^[:space:]]+')
PHASE_ARG=$(echo "$ARGUMENTS" | sed -E 's/--ws[[:space:]]+[^[:space:]]+//g' | xargs)

INIT=$(gtd-sdk query init.verify-work "${PHASE_ARG}" ${GTD_WS})
...
MVP_MODE=$(gtd-sdk query phase.mvp-mode "${phase_number}" ${GTD_WS} --pick active)
```
</step>

<step name="issue_driven_reconciliation_guard">
Before creating or resuming UAT, check whether this phase was exported to
GitHub task issues and has parent plans ready for reconciliation but missing
canonical summaries:

```bash
RECONCILE_CHECK=$(gtd-sdk query work-task-issue --phase "${phase_number}" --read-only 2>/dev/null || true)
```

If `RECONCILE_CHECK` reports `action: request_reconciliation_permission` or
has `reconciliation.ready: true`, block UAT with `ISSUE_TASK_RECONCILIATION_REQUIRED`.
List the ready parent plan issue numbers and tell the user to run
`/gtd:work-task-issue {parent_issue_or_plan_id} --phase {phase} --reconcile`
first. Do not infer UAT deliverables directly from task issues; `verify-work`
depends on canonical `*-SUMMARY.md` files and the phase `VERIFICATION.md`.
</step>
...
Provide a phase number to start testing (e.g., /gtd:verify-work 4)
```
...
```
UI_PHASE_FLAG=$(gtd-sdk query config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@~/.claude/get-tasks-done/references/verify-mvp-mode.md`. Briefly:
...
```bash
PHASE_GOAL=$(gtd-sdk query roadmap.get-phase "${phase_number}" ${GTD_WS} --pick goal)
USER_STORY_VALID=$(gtd-sdk query user-story.validate --story "$PHASE_GOAL" --pick valid)
if [ "$USER_STORY_VALID" != "true" ]; then
  echo "Phase ${phase_number} has '**Mode:** mvp' in ROADMAP.md but the **Goal:** is not in user-story format."
  echo "Run /gtd mvp-phase ${phase_number} to set a user-story goal before verifying."
  exit 1
fi
```
...
```bash
CHECKPOINT=$(gtd-sdk query uat.render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
gtd-sdk query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
```
⚠ Security enforcement enabled — /gtd:secure-phase {phase} has not run.
Run before advancing to the next phase.

All tests passed. Ready to continue.

- `/gtd:secure-phase {phase}` — security review (required before advancing)
- `/gtd:plan-phase {next}` — Plan next phase
- `/gtd:work-task-issue --phase {next}` — Execute next phase
- `/gtd:ui-review {phase}` — visual quality audit (if frontend files were modified)
```

If `SECURITY_CFG` is `true` AND `SECURITY_FILE` exists: check frontmatter `threats_open`. If > 0:
```
⚠ Security gate: {threats_open} threats open
  /gtd:secure-phase {phase} — resolve before advancing
```
...
Read and follow `~/.claude/get-tasks-done/workflows/transition.md`.
...
- `/gtd:plan-phase {next}` — Plan next phase
- `/gtd:work-task-issue --phase {next}` — Execute next phase
- `/gtd:secure-phase {phase}` — security review
- `/gtd:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
`audit-open` is CJS-only until registered on `gtd-sdk query`:

```bash
gtd-sdk query audit-open --json
```
...
If user declines: stop. User resolves items and re-runs `/gtd:verify-work`.
...
- Follow @~/.claude/get-tasks-done/workflows/diagnose-issues.md
...
Spawn gtd-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /gtd:work-task-issue --phase
...
Spawn gtd-planner with revision context:
...
3. Abandon (exit, user runs /gtd:plan-phase manually)
...
`/clear` then `/gtd:work-task-issue --phase {phase} --gaps-only`
...
- [ ] If issues: gtd-planner creates fix plans (gap_closure mode)
- [ ] If issues: gtd-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/gtd:work-task-issue --phase --gaps-only` when complete
</success_criteria>
