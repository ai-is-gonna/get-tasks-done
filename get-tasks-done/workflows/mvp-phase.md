<purpose>
Guide the user through MVP-mode planning for a phase. Prompts for an "As a / I want to / So that" user story, runs SPIDR splitting check on the story, writes the result to ROADMAP.md, and delegates to `/gtd plan-phase` (which auto-detects MVP via the roadmap mode field shipped in PRD Phase 1).
</purpose>

<required_reading>
@~/.claude/get-tasks-done/references/user-story-template.md
@~/.claude/get-tasks-done/references/spidr-splitting.md
@~/.claude/get-tasks-done/references/planner-mvp-mode.md
</required_reading>
...
Usage: /gtd mvp-phase <phase-number>
Example: /gtd mvp-phase 1
Example: /gtd mvp-phase 2.1
```
Exit.

Normalize per `@~/.claude/get-tasks-done/references/phase-argument-parsing.md` (zero-pad integer phases to two digits).
...
ANALYZE=$(gtd-sdk query roadmap.analyze)
...
If `PHASE_FOUND` is `false`: error and exit. Suggest `/gtd add-phase` or `/gtd insert-phase` to create the phase first.
...
To proceed anyway: /gtd mvp-phase ${PHASE} --force
```
...
Run three sequential `AskUserQuestion` calls. Each is free-text. After all three, assemble into the canonical sentence per `@~/.claude/get-tasks-done/references/user-story-template.md`:
...
```bash
USER_STORY_RESULT=$(gtd-sdk query user-story.validate --story "$USER_STORY")
...
Run the SPIDR rules from `@~/.claude/get-tasks-done/references/spidr-splitting.md`. Briefly:
...
- **Accept**: `USER_STORY` becomes the first split's story (`${HAPPY_STORY}` in the example). Surface the remaining splits as a list of `/gtd add-phase` invocations the user can run after this command completes — do NOT auto-create the new phases (preserve user control over numbering).
...
```bash
NEW_MODE=$(gtd-sdk query roadmap.get-phase "${PHASE}" --pick mode)
NEW_GOAL=$(gtd-sdk query roadmap.get-phase "${PHASE}" --pick goal)
```
...
## 7. Delegate to /gtd plan-phase

Invoke `/gtd plan-phase ${PHASE}` (no flags). Phase 1's MVP_MODE resolution chain (CLI flag → roadmap mode → config → false) will detect the new `**Mode:** mvp` line and run plan-phase in vertical-slice mode automatically.
...
>
> - `/gtd add-phase` — for the next slice: «${SPLIT_2_STORY}»
> - `/gtd add-phase` — for the next slice: «${SPLIT_3_STORY}»
>
> Each will be added to the end of the current milestone. You can then run
> `/gtd mvp-phase <new-phase-number>` on each to plan them as MVP slices."
