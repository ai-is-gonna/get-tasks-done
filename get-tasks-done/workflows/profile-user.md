...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `gtd-sdk query` handlers (with legacy `gtd-tools.cjs` parity where needed) and the gtd-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/get-tasks-done/references/ui-brand.md (display patterns)
- @$HOME/.claude/agents/gtd-user-profiler.md (profiler agent definition)
- @$HOME/.claude/get-tasks-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/get-tasks-done/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/get-tasks-done/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/get-tasks-done/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /gtd:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(gtd-sdk query scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(gtd-sdk query profile-sample --json 2>/dev/null)
```
...
**Spawn gtd-user-profiler agent using Task tool:**

Use the Task tool to spawn the `gtd-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/get-tasks-done/references/user-profiling.md`
...
Reference: @$HOME/.claude/get-tasks-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(gtd-sdk query profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/gtd-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(gtd-sdk query profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/gtd-profile-analysis-XXXXXX.json)
```
...
```bash
gtd-sdk query write-profile --input "$ANALYSIS_PATH" --json
```

Display: "✓ Profile written to $HOME/.claude/get-tasks-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/gtd-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/get-tasks-done/USER-PROFILE.md" and jump to step 10.
...
**For /gtd-dev-preferences (if selected):**

```bash
gtd-sdk query generate-dev-preferences --analysis "$ANALYSIS_PATH" --json
```

Display: "✓ Generated /gtd-dev-preferences at $HOME/.claude/skills/gtd-dev-preferences/SKILL.md"

**For CLAUDE.md profile section (if selected):**

```bash
gtd-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --json
```
...
```bash
gtd-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any `gtd-sdk query` or gtd-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/get-tasks-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gtd-dev-preferences   $HOME/.claude/skills/gtd-dev-preferences/SKILL.md
...
- [ ] Artifacts generated sequentially via gtd-sdk query (or gtd-tools.cjs) subcommands
