...
offers mid-conversation research when useful, then routes crystallized outputs to GTD artifacts.
</purpose>

<required_reading>
Read all files referenced by the invoking prompt's execution_context before starting.

@~/.claude/get-tasks-done/references/questioning.md
...
- gtd-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
| Spike | `/gtd:spike` (invoke) | Feasibility uncertainty surfaced — "will this API work?", "can we do X?" |
| Sketch | `/gtd:sketch` (invoke) | Design direction unclear — "what should this look like?", "how should this feel?" |
...
- **Phases:** Use existing `/gtd-add-phase` command via SlashCommand

Commit if `commit_docs` is enabled:
```bash
gtd-sdk query commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/gtd:explore` or start working with `/gtd:progress --next`.
```
