...
```bash
STATS=$(gtd-sdk query stats.json)
...
If no `.planning/` directory exists, inform the user to run `/gtd:new-project` first.
</step>

<step name="mvp_summary">
**MVP phase summary.** Read all phases via `gtd-sdk query roadmap.analyze` (Phase 1's `cmdRoadmapAnalyze` surfaces a `mode` field per phase). Count phases by mode:
