<purpose>
Create structured `.planning/HANDOFF.json` and `.continue-here.md` handoff files to preserve complete work state across sessions. The JSON provides machine-readable state for `/gtd:resume-work`; the markdown provides human-readable context.
</purpose>
...
9. **Blocking constraints**: Anti-patterns or methodological failures encountered during this session that a resuming agent MUST be aware of before proceeding. Only include items discovered through actual failure — not warnings or predictions. Assign each constraint a `severity`:
   - `blocking` — The resuming agent MUST demonstrate understanding before proceeding. The discuss-phase and task orchestration flow workflows will enforce a mandatory understanding check.
...
```bash
timestamp=$(gtd-sdk query current-timestamp full --raw)
```
...
_Remove rows that do not apply. The discuss-phase and task orchestration flow workflows parse this table and enforce a mandatory understanding check for any `blocking` rows._
...
<step name="commit">
```bash
gtd-sdk query commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
