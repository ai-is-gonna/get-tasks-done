<purpose>
Detect current project state and automatically advance to the next logical GTD workflow step.
...
```bash
# Get state snapshot
gtd-sdk query state.json 2>/dev/null || echo "{}"
```
...
```
No GTD project detected. Run `/gtd:new-project` to get started.
```
Exit.
</step>
...
Run `/gtd:health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
After passing all three hard-stop gates, scan all phases that precede the current phase in ROADMAP.md order for incomplete work. For each prior phase number `N`, use `gtd-sdk query find-phase <N>` JSON (plans, summaries, incomplete_plans, etc.) to inspect that phase.
...
**Source phase:** {src}
**Deferred at:** {date} during /gtd:progress --next advancement to Phase {dest}
**Plans:**
- [ ] {N}-{M}: {slug} (ran, no SUMMARY.md)
```
2. Commit the deferral record:
```bash
gtd-sdk query commit "docs: defer incomplete Phase {src} items to backlog"
```
...
⚠ Pending exploratory work:
  ...
  Resume with `/gtd:spike` or `/gtd:sketch`, or continue with phase work below.
```
...
→ Next action: `/gtd:discuss-phase <first-phase>`
...
▶ **Next step:** `/gtd-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/gtd:progress --next` is zero-friction advancement.
</step>
