...
```bash
INIT=$(gtd-sdk query init.milestone-op)
...
```bash
DETAIL=$(gtd-sdk query roadmap.get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(gtd-sdk query init.phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(gtd-sdk query config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
gtd-sdk query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="gtd-discuss-phase", args="${PHASE_NUM}")
```
...
```bash
PHASE_SECTION=$(gtd-sdk query roadmap.get-phase ${PHASE_NUM} 2>/dev/null)
...
```
Skill(skill="gtd-ui-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"gtd-plan-phase\", args=\"${PHASE_NUM}\")"
)
```
...
```
Skill(skill="gtd-plan-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  run_in_background=true,
  prompt="Run task issue workflow for phase ${PHASE_NUM}: Skill(skill=\"gtd-export-phase-issues\", args=\"${PHASE_NUM}\") then Skill(skill=\"gtd-work-task-issue\", args=\"--phase ${PHASE_NUM}\")"
)
```
...
```
Skill(skill="gtd-export-phase-issues", args="${PHASE_NUM}")
Skill(skill="gtd-work-task-issue", args="--phase ${PHASE_NUM}")
```

**3c.5. Code Review and Fix**

Auto-invoke code review and fix chain. Autonomous mode chains both review and fix (unlike task issue work and quick mode, which only suggest fixes).

**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(gtd-sdk query config-get workflow.code_review 2>/dev/null || echo "true")
```
If `"false"`: display "Code review skipped (workflow.code_review=false)" and proceed to 3d.

```
Skill(skill="gtd-code-review", args="${PHASE_NUM}")
```

Parse status from REVIEW.md frontmatter. If "clean" or "skipped": proceed to 3d. If findings found: auto-invoke:
```
Skill(skill="gtd-code-review", args="${PHASE_NUM} --fix --auto")
```
...
After task issue work and finalization return (or the background agent completes), read the verification result:
...
```
Skill(skill="gtd-plan-phase", args="${PHASE_NUM} --gaps")
```
...
```bash
UI_REVIEW_CFG=$(gtd-sdk query config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```
Skill(skill="gtd-ui-review", args="${PHASE_NUM}")
```
...
> Full instructions are in `get-tasks-done/references/autonomous-smart-discuss.md`. Read that file now and follow it exactly.

Smart discuss is an autonomous-optimized variant of `gtd-discuss-phase`. It proposes grey area answers in batch tables — the user accepts or overrides per area — and writes an identical CONTEXT.md to what discuss-phase produces.

**Inputs:** `PHASE_NUM` from execute_phase.

Read and execute: `$HOME/.claude/get-tasks-done/references/autonomous-smart-discuss.md`
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► AUTONOMOUS ▸ --to ${TO_PHASE} REACHED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Completed through phase ${TO_PHASE} as requested.
 Remaining phases were not executed.

 Resume with: /gtd:autonomous --from ${next_incomplete_phase}
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► AUTONOMOUS ▸ PHASE ${ONLY_PHASE} COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Phase ${ONLY_PHASE}: ${PHASE_NAME} — Done
 Mode: Single phase (--only)

 Lifecycle skipped — run /gtd:autonomous without --only
...
```
Skill(skill="gtd-audit-milestone")
```
...
```
Skill(skill="gtd-complete-milestone", args="${milestone_version}")
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /gtd:autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}${TO_PHASE ? " --to " + TO_PHASE : ""}
```
...
<success_criteria>
- [ ] All incomplete phases completed in order (smart discuss → ui-phase → plan → task issue work → ui-review each)
...
- [ ] Task issue work invoked through export-phase-issues and work-task-issue
...
- [ ] Plan-phase and task issue work failures route to handle_blocker
...
- [ ] Audit invoked via Skill(skill="gtd-audit-milestone")
...
- [ ] `--interactive` runs discuss inline via gtd-discuss-phase (asks questions, waits for user)
