...
- gtd-debugger — Diagnoses and fixes issues
</available_agent_types>
...
```bash
AGENT_SKILLS_DEBUGGER=$(gtd-sdk query agent-skills gtd-debugger)
...
```bash
gtd-sdk query commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
