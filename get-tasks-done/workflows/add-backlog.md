...
Invoked by `/gtd:capture --backlog` (`commands/gtd/capture.md`).
...
```bash
NEXT=$(gtd-sdk query phase.next-decimal 999 --raw)
```
...
Plans:
- [ ] TBD (promote with /gtd:review-backlog when ready)
```
...
PROJECT_CODE=$(gtd-sdk query config-get project_code --raw 2>/dev/null || echo "")
...
```bash
gtd-sdk query commit "docs: add backlog item ${NEXT} — ${ARGUMENTS}" --files .planning/ROADMAP.md "${PHASE_DIR}/.gitkeep"
```
...
Use /gtd:discuss-phase {NEXT} to explore it further.
Use /gtd:review-backlog to promote items to active milestone.
```
...
- Phase directories are created immediately so /gtd:discuss-phase and /gtd:plan-phase work on them
- No `Depends on:` field — backlog items are unsequenced by definition
- Sparse numbering is fine (999.1, 999.3) — always uses next-decimal
- Promote backlog items to the active milestone with /gtd:review-backlog
</notes>
