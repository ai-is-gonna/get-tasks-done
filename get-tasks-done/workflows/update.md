<purpose>
Check for GTD updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether GTD is installed locally or globally by checking both locations and validating install integrity.

First, derive `PREFERRED_CONFIG_DIR` and `PREFERRED_RUNTIME` from the invoking prompt's `execution_context` path:
- If the path contains `/get-tasks-done/workflows/update.md`, strip that suffix and store the remainder as `PREFERRED_CONFIG_DIR`
...
# runtime directories.
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/get-tasks-done/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/get-tasks-done/workflows/update.md" ]; }; then
  ...
  if [ -f "$PREFERRED_CONFIG_DIR/get-tasks-done/VERSION" ] && grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+' "$PREFERRED_CONFIG_DIR/get-tasks-done/VERSION"; then
    INSTALLED_VERSION="$(cat "$PREFERRED_CONFIG_DIR/get-tasks-done/VERSION")"
  else
  ...
  # GTD_DIR or downstream check_latest_version misreads the install as
  # UNKNOWN. PREFERRED_CONFIG_DIR is the resolved config dir we just
  # validated above (line 95-96); it is the right GTD_DIR value for
  # this fast path.
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/get-tasks-done/VERSION" ] || [ -f "./$dir/get-tasks-done/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/get-tasks-done/VERSION"
    LOCAL_MARKER_FILE="./$dir/get-tasks-done/workflows/update.md"
...
for entry in "${ORDERED_ENV_RUNTIME_DIRS[@]}"; do
  ...
  if [ -f "$dir/get-tasks-done/VERSION" ] || [ -f "$dir/get-tasks-done/workflows/update.md" ]; then
    ...
    GLOBAL_VERSION_FILE="$dir/get-tasks-done/VERSION"
    GLOBAL_MARKER_FILE="$dir/get-tasks-done/workflows/update.md"
...
if [ -z "$GLOBAL_RUNTIME" ]; then
  for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
    runtime="${entry%%:*}"
    dir="${entry#*:}"
    if [ -f "$HOME/$dir/get-tasks-done/VERSION" ] || [ -f "$HOME/$dir/get-tasks-done/workflows/update.md" ]; then
      GLOBAL_RUNTIME="$runtime"
      GLOBAL_VERSION_FILE="$HOME/$dir/get-tasks-done/VERSION"
      GLOBAL_MARKER_FILE="$HOME/$dir/get-tasks-done/workflows/update.md"
...
- Line 4 = resolved GTD config dir (e.g. `/Users/me/.claude`, `/Users/me/.gemini`); empty if scope is `UNKNOWN`. Capture this as `GTD_DIR` and pass it to subsequent steps so they don't have to re-derive the runtime path.
...
The `GTD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`~/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. If `GTD_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.

`LATEST_RESULT` is a JSON document with the documented shape `{ ok: bool, version: string, reason: string, detail?: string }`. Parse via `jq` ONLY when the script actually ran. When `GTD_DIR` is empty (scope `UNKNOWN`), skip the check entirely and seed the parsed fields with their no-op values so downstream logic does not mistake an unset `LATEST_RESULT` for a failed network check (#2993 CR feedback):
...
if [ -z "$GTD_DIR" ]; then
  ...
  LATEST_RESULT="$(node "$GTD_DIR/get-tasks-done/bin/check-latest-version.cjs" --json 2>/dev/null)"
  LATEST_STATUS=$?
...
To update manually: `npx -y --package=get-tasks-done@latest -- get-tasks-done --global`
```
...
Running /gtd:update would install the npm release (A.B.C) and downgrade
...
⚠️  **Note:** The installer performs a clean install of GTD folders:
- `commands/gtd/` will be wiped and replaced
- `get-tasks-done/` will be wiped and replaced
- `agents/gtd-*` files will be replaced
...
- Custom commands not in `commands/gtd/` ✓
- Custom agents not prefixed with `gtd-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any GTD files directly, they'll be automatically backed up to `gtd-local-patches/` and can be reapplied with `/gtd:update --reapply` after the update.
```
...
GTD-managed directories. These are files that exist on disk but are NOT listed
in `gtd-file-manifest.json` — i.e., files the user added themselves that the
...
even when custom files exist (bug #1997). Use `gtd-sdk query detect-custom-files`
when `gtd-sdk` is on `PATH`, or the bundled `gtd-tools.cjs detect-custom-files`
...
```bash
GTD_TOOLS="$RUNTIME_DIR/get-tasks-done/bin/gtd-tools.cjs"
CUSTOM_JSON=''
if [ -n "$RUNTIME_DIR" ] && command -v gtd-sdk >/dev/null 2>&1; then
  CUSTOM_JSON=$(gtd-sdk query detect-custom-files --config-dir "$RUNTIME_DIR" 2>/dev/null)
elif [ -f "$GTD_TOOLS" ] && [ -n "$RUNTIME_DIR" ]; then
  CUSTOM_JSON=$(node "$GTD_TOOLS" detect-custom-files --config-dir "$RUNTIME_DIR" 2>/dev/null)
fi
...
Back up each custom file to `$RUNTIME_DIR/gtd-user-files-backup/` before the
installer wipes the directories:

```bash
BACKUP_DIR="$RUNTIME_DIR/gtd-user-files-backup"
mkdir -p "$BACKUP_DIR"
...
```
⚠️  Found N custom file(s) inside GTD-managed directories.
    These have been backed up to gtd-user-files-backup/ before the update.
...
**If LOCAL install:**
```bash
npx -y --package=get-tasks-done@latest -- get-tasks-done "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y --package=get-tasks-done@latest -- get-tasks-done "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y --package=get-tasks-done@latest -- get-tasks-done --claude --global
```
...
for dir in "${CACHE_DIRS[@]}"; do
  if [ -n "$dir" ]; then
    rm -f "$dir/cache/gtd-update-check.json"
  fi
done

for dir in .claude .config/opencode .opencode .gemini/antigravity .gemini .config/kilo .kilo .codex; do
  rm -f "./$dir/cache/gtd-update-check.json"
  rm -f "$HOME/$dir/cache/gtd-update-check.json"
done

# Clear the shared tool-agnostic cache written by gtd-check-update.js hook (#2784).
# The hook uses ~/.cache/gtd/gtd-update-check.json regardless of runtime; clear it
# so the statusline stops showing the stale "⬆ /gtd:update" indicator after update.
rm -f "$HOME/.cache/gtd/gtd-update-check.json"
```

The SessionStart hook (`gtd-check-update.js`) writes to the detected runtime's cache directory, so preferred/env-derived paths and default paths must all be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  GTD Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/ai-is-gonna/get-tasks-done/blob/main/CHANGELOG.md)
```
</step>
...
Check for gtd-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run `/gtd:update --reapply` to merge your modifications into the new version.
```
