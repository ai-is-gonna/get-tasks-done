...
Saves artifacts to `.planning/spikes/`. Companion to `/gtd:spike --wrap-up`.
...
```bash
gtd-sdk query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files .planning/spikes/NNN-descriptive-name/ .planning/spikes/MANIFEST.md
```
...
```bash
gtd-sdk query commit "docs(spikes): update conventions" --files .planning/spikes/CONVENTIONS.md
```
</step>
...
**Also available:**
- `/gtd:spike` — spike more ideas (or run with no argument for frontier mode)
- `/gtd:plan-phase` — start planning the real implementation
- `/gtd:explore` — continue exploring the idea
