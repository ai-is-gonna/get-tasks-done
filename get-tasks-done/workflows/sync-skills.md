# sync-skills — Cross-Runtime GTD Skill Sync

**Command:** `/gtd-sync-skills`

Sync managed `gtd-*` skill directories from one canonical runtime's skills root to one or more destination runtime skills roots. Keeps multi-runtime installs aligned after a `gtd-update` on one runtime.
...
```bash
INSTALL_JS="$(dirname "$0")/../get-tasks-done/bin/install.js"
# If running from a global install, resolve relative to the GTD package
INSTALL_JS_GLOBAL="$HOME/.claude/get-tasks-done/bin/install.js"
...
error: source skills root not found: <path>
       Is GTD installed globally for the '<runtime>' runtime?
       Run: node ~/.claude/get-tasks-done/bin/install.js --global --<runtime>
```
Then exit.
...
```bash
# List gtd-* subdirectories in source
SRC_SKILLS=$(ls -1 "$SRC_SKILLS_ROOT" 2>/dev/null | grep '^gtd-')

# List gtd-* subdirectories in destination (may not exist yet)
DST_SKILLS=$(ls -1 "$DEST_ROOT" 2>/dev/null | grep '^gtd-')

# Diff:
# CREATE  — in SRC but not in DST
# UPDATE  — in both; content differs (compare recursively via checksums)
# REMOVE  — in DST but not in SRC (stale GTD skill no longer in source)
# SKIP    — in both; content identical (already up to date)
```

**Non-GTD preservation:** Only `gtd-*` entries are ever created, updated, or removed. Entries in the destination that do not start with `gtd-` are never touched.
...
CREATE: gtd-help
UPDATE: gtd-update
REMOVE: gtd-old-command
SKIP:   gtd-plan-phase (up to date)
(N changes)
...
1. **Only `gtd-*` directories** are created, updated, or removed. Any directory not starting with `gtd-` in a destination root is untouched.
2. **Dry-run is the default.** `--apply` must be passed explicitly to write anything.
3. **Source root must exist.** Never create the source root; it must have been created by a prior `gtd-update` or installer run.
...
- Sync copies files verbatim and does not apply runtime-specific content transformations. Use the GTD installer directly for runtimes that require format conversion.
