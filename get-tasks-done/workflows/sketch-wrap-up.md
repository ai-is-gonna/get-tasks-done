...
Run `/gtd:sketch` first to create design explorations.
```
Exit.
...
```bash
gtd-sdk query commit "docs(sketch-wrap-up): package [N] sketch findings into project skill" --files .planning/sketches/WRAP-UP-SUMMARY.md
```
</step>
...
`/gtd:sketch` (run with no argument — its frontier mode analyzes the sketch landscape and proposes consistency and frontier sketches)

───────────────────────────────────────────────────────────────

**Also available:**
- `/gtd:plan-phase` — start building the real UI
- `/gtd:ui-phase` — generate a UI design contract for a frontend phase
- `/gtd:sketch [idea]` — sketch a specific new design area
- `/gtd:explore` — continue exploring
...
- [ ] Summary presented
- [ ] Next-step options presented (including frontier sketch exploration via `/gtd:sketch`)
</success_criteria>
