...
- gtd-project-researcher — Researches project-level technical decisions
- gtd-research-synthesizer — Synthesizes findings from parallel research agents
- gtd-roadmapper — Creates phased execution roadmaps
</available_agent_types>
...
```bash
INIT=$(gtd-sdk query init.new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(gtd-sdk query agent-skills gtd-project-researcher)
AGENT_SKILLS_SYNTHESIZER=$(gtd-sdk query agent-skills gtd-research-synthesizer)
AGENT_SKILLS_ROADMAPPER=$(gtd-sdk query agent-skills gtd-roadmapper)
```
...
```text
⚠ GTD agents not installed. The following agents are missing from your agents directory:
...
Subagent spawns (gtd-project-researcher, gtd-research-synthesizer, gtd-roadmapper) will fail
with "agent type not found" if `required_agents_installed` is false. Run the installer with --global to make agents available:

  npx @ai-is-gonna/get-tasks-done@latest --global
...
```bash
mkdir -p .planning
gtd-sdk query config-new-project '{"mode":"yolo","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":true|false,"auto_advance":true}}'
```
...
```bash
mkdir -p .planning
gtd-sdk query commit "chore: add project config" --files .planning/config.json
```
...
{if HAS_SPIKES && !SPIKE_SKILL}  ◆ Raw spikes in .planning/spikes/ — consider `/gtd:spike --wrap-up` to package findings
{if HAS_SKETCHES && !SKETCH_SKILL}  ◆ Raw sketches in .planning/sketches/ — consider `/gtd:sketch --wrap-up` to package findings
...
```bash
mkdir -p .planning
gtd-sdk query commit "docs: initialize project" --files .planning/PROJECT.md
```
...
**Check for global defaults** at `~/.gtd/defaults.json`. If the file exists, read and display its contents before asking:

```bash
DEFAULTS_RAW=$(cat ~/.gtd/defaults.json 2>/dev/null)
```
...
```text
Your saved defaults (~/.gtd/defaults.json):
  • Mode: [value]
  • Granularity: [value]
...
**If "Configure fresh" or `~/.gtd/defaults.json` doesn't exist:** proceed with the questions below.
...
```bash
mkdir -p .planning
gtd-sdk query config-new-project '{"mode":"[yolo|interactive]","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":[false if granularity=coarse, true otherwise]}}'
```
...
- question: "I detected separate git repos in this workspace. Which directories contain code that GTD should commit to?"
- multiSelect: true
...
Spawn 4 parallel gtd-project-researcher agents with path references:
...
Use template: ~/.claude/get-tasks-done/templates/research-project/STACK.md
</output>
", subagent_type="gtd-project-researcher", model="{researcher_model}", description="Stack research")
...
Use template: ~/.claude/get-tasks-done/templates/research-project/FEATURES.md
</output>
", subagent_type="gtd-project-researcher", model="{researcher_model}", description="Features research")
...
Use template: ~/.claude/get-tasks-done/templates/research-project/ARCHITECTURE.md
</output>
", subagent_type="gtd-project-researcher", model="{researcher_model}", description="Architecture research")
...
Use template: ~/.claude/get-tasks-done/templates/research-project/PITFALLS.md
</output>
", subagent_type="gtd-project-researcher", model="{researcher_model}", description="Pitfalls research")
```
...
Use template: ~/.claude/get-tasks-done/templates/research-project/SUMMARY.md
Commit after writing.
</output>
", subagent_type="gtd-research-synthesizer", model="{synthesizer_model}", description="Synthesize research")
```
...
```bash
gtd-sdk query commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
Spawn gtd-roadmapper agent with path references:
...
", subagent_type="gtd-roadmapper", model="{roadmapper_model}", description="Create roadmap")
```
...
- Re-spawn roadmapper with revision context:
  ...
  ", subagent_type="gtd-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
```bash
gtd-sdk query generate-claude-md --output "$INSTRUCTION_FILE"
```

This ensures new projects get the default GTD workflow-enforcement guidance and current project context in `$INSTRUCTION_FILE`.

**Commit roadmap (after approval or auto mode):**

```bash
gtd-sdk query commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md "$INSTRUCTION_FILE"
```
...
```bash
PHASE1_SECTION=$(gtd-sdk query roadmap.get-phase 1 2>/dev/null)
...
- [ ] gtd-roadmapper spawned with context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] ROADMAP.md created with phases, requirement mappings, success criteria
- [ ] STATE.md initialized
- [ ] REQUIREMENTS.md traceability updated
- [ ] `$INSTRUCTION_FILE` generated with GTD workflow guidance (AGENTS.md for Codex, CLAUDE.md otherwise)
