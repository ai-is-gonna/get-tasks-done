...
Executed by a verification subagent spawned from work-task-issue.md.
</purpose>
...
<required_reading>
@~/.claude/get-tasks-done/references/verification-patterns.md
@~/.claude/get-tasks-done/templates/verification-report.md
</required_reading>
...
Use `gtd-sdk query` verify handlers (or legacy gtd-tools) to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(gtd-sdk query frontmatter.get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(gtd-sdk query roadmap.get-phase "${phase_number}" --raw)
```
...
Use `gtd-sdk query verify.artifacts` (or legacy gtd-tools) for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(gtd-sdk query verify.artifacts "$plan")
...
Use `gtd-sdk query verify.key-links` (or legacy gtd-tools) for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(gtd-sdk query verify.key-links "$plan")
...
if [ "$GATE_CFG" != "false" ]; then
  ...
  DECISION_RESULT=$(gtd-sdk query check.decision-coverage-verify "${PHASE_DIR}" "${CONTEXT_PATH}")
fi
```
...
TEST_CMD=$(gtd-sdk query config-get workflow.test_command --default "" 2>/dev/null || true)
if [ -z "$TEST_CMD" ]; then
...
See ~/.claude/get-tasks-done/templates/verification-report.md for complete template.
</step>
