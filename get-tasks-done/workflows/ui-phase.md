<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates gtd-ui-researcher and gtd-ui-checker with a revision loop. Inserts between discuss-phase and plan-phase in the lifecycle.
...
- gtd-ui-researcher — Researches UI/UX approaches
- gtd-ui-checker — Reviews UI implementation quality
</available_agent_types>
...
AGENT_SKILLS_UI=$(gtd-sdk query agent-skills gtd-ui-researcher)
AGENT_SKILLS_UI_CHECKER=$(gtd-sdk query agent-skills gtd-ui-checker)
```
...
```bash
UI_RESEARCHER_MODEL=$(gtd-sdk query resolve-model gtd-ui-researcher --raw)
UI_CHECKER_MODEL=$(gtd-sdk query resolve-model gtd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(gtd-sdk query config-get workflow.ui_phase 2>/dev/null || echo "true")
```

**If `UI_ENABLED` is `false`:**
```
UI phase is disabled in config. Enable via /gtd:settings.
```
Exit workflow.
...
Recommended: run /gtd:discuss-phase {N} first to capture design preferences.
...
⚡ Sketch findings detected: {SKETCH_FINDINGS_PATH}
   Validated design decisions from /gtd:sketch will be loaded into the UI researcher.
...
## 5. Spawn gtd-ui-researcher

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► UI DESIGN CONTRACT — PHASE {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read ~/.claude/agents/gtd-ui-researcher.md for instructions.
...
- {SKETCH_FINDINGS_PATH} (Sketch Findings — validated design decisions, CSS patterns, visual direction from /gtd:sketch, if exists)
</files_to_read>
...
Template: ~/.claude/get-tasks-done/templates/UI-SPEC.md
</output>
...
Agent(
  ...
  subagent_type="gtd-ui-researcher",
...
## 7. Spawn gtd-ui-checker

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► VERIFYING UI-SPEC
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read ~/.claude/agents/gtd-ui-checker.md for instructions.
...
- Re-spawn gtd-ui-researcher with revision context:
...
2. Edit manually — open UI-SPEC.md in editor, re-run /gtd:ui-phase
...
`/clear` then: `/gtd:plan-phase {N}`

{If CONTEXT.md does NOT exist:}
**Discuss Phase {N}** — gather implementation context before planning

`/clear` then: `/gtd:discuss-phase {N}`

(or `/gtd:plan-phase {N}` to skip discussion)
...
```bash
gtd-sdk query commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
...
- [ ] gtd-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] gtd-ui-checker spawned with UI-SPEC.md
