<purpose>
Verify milestone achieved its definition of done by aggregating phase verifications, checking cross-phase integration, and assessing requirements coverage. Reads existing VERIFICATION.md files (phases already verified during task orchestration flow), aggregates tech debt and deferred gaps, then spawns integration checker for cross-phase wiring.
</purpose>
...
- gtd-integration-checker — Checks cross-phase integration
</available_agent_types>
...
AGENT_SKILLS_CHECKER=$(gtd-sdk query agent-skills gtd-integration-checker)
```
...
```bash
integration_checker_model=$(gtd-sdk query resolve-model gtd-integration-checker --raw)
```
...
PHASE_INFO=$(gtd-sdk query find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="gtd-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  gtd-sdk query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(gtd-sdk query config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/gtd:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/gtd:validate-phase {N}` |

Phases needing validation: run `/gtd:validate-phase {N}` for each flagged phase.
...
/gtd:phase --insert <N> "Close gap: <REQ-ID> — <description>"
/gtd:discuss-phase <N>
/gtd:plan-phase <N>
/gtd:work-task-issue --phase <N>

For Nyquist-coverage gaps flagged in the table above, prefer running
`/gtd:validate-phase <N>` for each flagged phase (and `/gtd:secure-phase
...
- /gtd:complete-milestone {version} — proceed anyway (accept tech debt)
...
/gtd:phase --insert <N> "Address tech debt: <area>"
/gtd:discuss-phase <N>
/gtd:plan-phase <N>
/gtd:work-task-issue --phase <N>
