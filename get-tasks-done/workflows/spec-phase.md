...
```bash
INIT=$(node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" init phase-op "${PHASE}")
...
If "Skip": Exit with message: "Existing SPEC.md unchanged. Run /gtd:discuss-phase [X] to continue."
...
Use the SPEC.md template from @~/.claude/get-tasks-done/templates/spec.md.
...
- User directed to /gtd:discuss-phase as next step
</success_criteria>
