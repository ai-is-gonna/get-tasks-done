<purpose>
Create all phases necessary to close gaps identified by `/gtd:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/gtd-add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/gtd:audit-milestone` first.
```
...
HIGHEST=$(gtd-sdk query phases.list --pick directories[-1])
```
...
```bash
gtd-sdk query commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/gtd:work-task-issue --phase {N}` — if plans already exist
...
`/gtd:audit-milestone` — re-audit to verify gaps closed
`/gtd:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/gtd:plan-phase` next
</success_criteria>
