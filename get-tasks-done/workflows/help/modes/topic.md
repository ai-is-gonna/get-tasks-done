...
| `map`, `map-codebase` | The `/gtd:map-codebase` block under `### Project Initialization` |
| `discuss`, `discuss-phase` | The `/gtd:discuss-phase` block under `### Phase Planning` |
| `plan`, `planning`, `plan-phase` | `### Phase Planning` |
| `execute`, `exec`, `task orchestration flow` | `### Execution` |
| `progress`, `route` | `### Progress Tracking` plus `### Smart Router` |
| `quick`, `quick-mode` | `### Quick Mode` |
| `fast` | The `/gtd:fast` block under `### Quick Mode` |
...
| `spike` | The `/gtd:spike` and `/gtd:spike --wrap-up` blocks under `### Spiking & Sketching` |
| `sketch` | The `/gtd:sketch` and `/gtd:sketch --wrap-up` blocks under `### Spiking & Sketching` |
| `spike-sketch`, `experiments` | `### Spiking & Sketching` |
| `capture`, `notes`, `todos` | `### Capturing Ideas, Notes, and Todos` |
| `verify`, `verify-work`, `uat` | `### User Acceptance Testing` plus the `/gtd:audit-uat` block |
| `pr` | `### Execution` plus the `/gtd:pr-branch` block |
| `review`, `peer-review` | The `/gtd:review` block under `### User Acceptance Testing` |
| `audit`, `auditing`, `audit-milestone` | `### Milestone Auditing` |
| `config`, `settings`, `configuration` | `### Configuration` |
| `cleanup` | The `/gtd:cleanup` block under `### Utility Commands` |
| `update` | The `/gtd:update` block under `### Utility Commands` |
...
3. If no match: emit a one-line error followed by a comma-separated list of the canonical topic names from the leftmost column (one per row, deduplicated). Suggest `/gtd:help --full` for the complete reference. Stop.
...
5. Read `workflows/help/modes/full.md`. Strip `<reference>` / `</reference>` wrapper tags — never emit them. Apply the extraction rule for the matched table cell, modulated by scope:
   ...
   - *Compact scope:* emit the heading, then the first `` **`/gtd:...`** `` bold line within the section (the signature) and the single non-blank line immediately after it (the one-line summary). If the section has no `` **`/gtd:...`** `` bold line, emit the heading and the first paragraph.
   ...
   5c. **Sub-block** (cell says `the /gtd:X block under ### Heading` or `the /gtd:X ... blocks under ### Heading`): within the named heading's section, start at each `` **`/gtd:X ...`** `` bold line.
   - *Full scope:* stop immediately before the next `` **`/gtd:...`** `` bold line or the next heading, whichever comes first.
...
6. After the section content, emit a single closing line:
   ...
   ```text
   More: /gtd:help --full · /gtd:help <topic> · /gtd:help --brief <topic>
   ```
