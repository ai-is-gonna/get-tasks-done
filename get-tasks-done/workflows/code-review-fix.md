<purpose>
Auto-fix issues from REVIEW.md. Validates phase, checks config gate, verifies REVIEW.md exists and has fixable issues, spawns gtd-code-fixer agent, handles --auto iteration loop (capped at 3), commits REVIEW-FIX.md once at the end, and presents results.
</purpose>
...
- gtd-code-fixer: Applies fixes to code review findings
- gtd-code-reviewer: Reviews source files for bugs and issues
</available_agent_types>
...
if [ ! -f "${REVIEW_PATH}" ]; then
  echo "Error: No REVIEW.md found for Phase ${PHASE_ARG}. Run /gtd:code-review ${PHASE_ARG} first."
  exit 1
fi
```
...
Spawn the gtd-code-fixer agent with config:
...
```text
Agent(subagent_type="gtd-code-fixer", prompt="
<files_to_read>
${REVIEW_PATH}
</files_to_read>
...
You can retry with /gtd:code-review ${PHASE_ARG} --fix.
```
...
if [ "$AUTO_MODE" = "true" ]; then
  ...
  - ${f}"
    ...
    # Spawn gtd-code-reviewer agent to re-review
...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: /gtd:code-review ${PHASE_ARG} --fix"
  echo ""
  echo "═══════════════════════════════════════════════════════════════"
  exit 1
fi
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  /gtd:verify-work  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  /gtd:code-review ${PHASE_NUMBER}           — Re-review code"
  echo "  /gtd:verify-work                           — Verify phase completion"
  echo ""
fi
```
