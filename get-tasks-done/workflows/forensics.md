...
Post-mortem investigation for failed or stuck GTD workflows. Analyzes git history,
...
> 'task orchestration flow failed silently', 'costs seem unusually high'."
...
Run the same comparison as the task orchestration flow safe-resume verifier: identify the
...
3. {Recovery command if applicable — e.g., `/gtd:resume-work`, `/gtd:work-task-issue --phase N`}

---

*Report generated by `/gtd:forensics`. All paths redacted for portability.*
```
...
BUG_LABEL=$(gh label list --repo ai-is-gonna/get-tasks-done --search "bug" --json name -q '.[0].name' 2>/dev/null)
LABEL_FLAG=""
if [ -n "$BUG_LABEL" ]; then
...
gh issue create \
  --repo ai-is-gonna/get-tasks-done \
