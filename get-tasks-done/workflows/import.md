...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via gtd-plan-checker
...
- If `--prd` is present: display message that `--prd` is not yet implemented and exit:
  ```
  GTD > --prd mode is planned for a future release. Use --from to import plan files.
  ```
- If neither flag is found: display usage and exit:

```
Usage: /gtd:import --from <path>

  --from <path>   Import an external plan file into GTD format
```
...
2. Read `.planning/PROJECT.md` — extract project constraints, tech stack, scope boundaries.
   **If PROJECT.md does not exist:** skip constraint checks that rely on it and display:
   ```
   GTD > Note: No PROJECT.md found. Conflict checks against project constraints will be skipped.
   ```
...
- **GTD PLAN.md format**: Has YAML frontmatter with `phase:`, `plan:`, `type:` fields
...
Convert the imported content to GTD PLAN.md format.
...
If the imported plan references PBR plan naming (e.g., `PLAN-01.md`, `plan-01.md`), rename all references to GTD `{NN}-{MM}-PLAN.md` convention during conversion.

Apply GTD naming convention for the output filename:
...
```bash
INIT=$(gtd-sdk query init.phase-op "{NN}")
...
Delegate validation to gtd-plan-checker:

```
Agent({
  subagent_type: "gtd-plan-checker",
  prompt: "Validate: .planning/phases/{phase}/{plan}-PLAN.md — check frontmatter completeness, task structure, and GTD conventions. Report any issues."
})
```
...
```bash
gtd-sdk query commit "docs({phase}): import plan from {basename FILEPATH}" --files .planning/phases/{phase}/{plan}-PLAN.md .planning/ROADMAP.md
```
...
- Use `pbr:plan-checker` or `pbr:planner` — use `gtd-plan-checker` and `gtd-planner`
- Write `.planning/.active-skill` — this is a PBR pattern with no GTD equivalent
