...
```bash
INIT=$(gtd-sdk query init.progress)
...
Run /gtd:new-project to start a new project.
```

Exit.

If missing STATE.md: suggest `/gtd:new-project`.

**If ROADMAP.md missing but PROJECT.md exists:**

This means a milestone was completed and archived. Go to **Route F** (between milestones).

If missing both ROADMAP.md and PROJECT.md: suggest `/gtd:new-project`.
</step>

<step name="load">
**Use structured extraction from `gtd-sdk query`:**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(gtd-sdk query roadmap.analyze)`
- `STATE=$(gtd-sdk query state-snapshot)`
...
<step name="issue_driven_reconciliation_route">
Check for exported GitHub task issue manifests before recommending native
execute/verify actions:

```bash
TASK_ISSUE_STATE=$(gtd-sdk query work-task-issue --read-only 2>/dev/null || true)
```

If this reports `request_reconciliation_permission` or ready reconciliation
plans, make reconciliation the next action. Show the exact
`/gtd:work-task-issue {parent_issue_or_plan_id} --reconcile` command for each
ready parent plan. Do not recommend `/gtd:verify-work` until the corresponding
`*-SUMMARY.md` files exist for an active issue-driven task flow.
</step>
...
- Use `summary-extract` for efficient parsing:
  ```bash
  gtd-sdk query summary-extract <path> --fields one_liner
  ```
...
**Generate progress bar from `gtd-sdk query progress` / `progress.json`, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(gtd-sdk query progress.bar --raw)
```
...
## Pending Todos
- [count] pending — /gtd:capture --list to review

## Active Debug Sessions
- [count] active — /gtd:debug to continue
...
```bash
MVP_MODE=$(gtd-sdk query phase.mvp-mode "${PHASE_NUMBER}" --pick active)
```
...
```bash
DEBT=$(gtd-sdk query audit-uat --raw 2>/dev/null)
```
...
Review: `/gtd:audit-uat ${GTD_WS}` — full cross-phase audit
Resume testing: `/gtd:verify-work {phase} ${GTD_WS}` — retest specific phase
```
...
2. `/gtd:work-task-issue --phase {phase} ${GTD_WS}` or `/gtd:orchestrate-tasks <child-issue>... ${GTD_WS}`
...
```bash
PHASE_SECTION=$(gtd-sdk query roadmap.get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/gtd:plan-phase {phase-number} ${GTD_WS}`
...
`/gtd:discuss-phase {phase}` — gather context and clarify approach

---

**Also available:**
- `/gtd:ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/gtd:plan-phase {phase}` — skip discussion, plan directly
- `/gtd:discuss-phase {phase}` — include assumptions check before planning
...
`/gtd:discuss-phase {phase} ${GTD_WS}` — gather context and clarify approach

---

**Also available:**
- `/gtd:plan-phase {phase} ${GTD_WS}` — skip discussion, plan directly
- `/gtd:discuss-phase {phase} ${GTD_WS}` — include assumptions check before planning
...
`/gtd:plan-phase {phase} --gaps ${GTD_WS}`

---

**Also available:**
- `/gtd:export-phase-issues {phase} ${GTD_WS}` — export updated gap plans to task issues
- `/gtd:work-task-issue --phase {phase} ${GTD_WS}` — work exported gap task issues
- `/gtd:verify-work {phase} ${GTD_WS}` — run more UAT testing
...
`/gtd:verify-work {phase} ${GTD_WS}` — resume testing from where you left off

---

**Also available:**
- `/gtd:audit-uat ${GTD_WS}` — full cross-phase UAT audit
- `/gtd:export-phase-issues {phase} ${GTD_WS}` — export planned task issues
- `/gtd:work-task-issue --phase {phase} ${GTD_WS}` — work exported task issues
...
```bash
NEXT_PHASE_SECTION=$(gtd-sdk query roadmap.get-phase "$((Z+1))" 2>/dev/null)
...
`/gtd:discuss-phase {Z+1}` — gather context and clarify approach

---

**Also available:**
- `/gtd:ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
- `/gtd:plan-phase {Z+1}` — skip discussion, plan directly
- `/gtd:verify-work {Z}` — user acceptance test before continuing
...
`/gtd:discuss-phase {Z+1} ${GTD_WS}` — gather context and clarify approach

---

**Also available:**
- `/gtd:plan-phase {Z+1} ${GTD_WS}` — skip discussion, plan directly
- `/gtd:verify-work {Z} ${GTD_WS}` — user acceptance test before continuing
...
`/gtd:complete-milestone ${GTD_WS}`

---

**Also available:**
- `/gtd:verify-work ${GTD_WS}` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/gtd:plan-phase [next] ${GTD_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/gtd:resume-work ${GTD_WS}`
</step>
...
- Check 2 (orphaned handoff): `Read the handoff file(s) and resume from where work was paused: /gtd:resume-work ${GTD_WS}`
...
- Check 1 (STATE inconsistency): `Run /gtd:verify-work ${PHASE} ${GTD_WS} to reconcile state`
</step>
...
- [ ] Smart routing: export/task issue route if plans exist, /gtd:plan-phase if not
- [ ] User confirms before any action
- [ ] Seamless handoff to appropriate gtd command
      </success_criteria>
