...
`.planning/spikes/WRAP-UP-SUMMARY.md`. Companion to `/gtd:spike`.
</purpose>
...
Run `/gtd:spike` first to create experiments.
```
Exit.
...
```bash
gtd-sdk query commit "docs(spike-wrap-up): package [N] spike findings into project skill" --files .planning/spikes/WRAP-UP-SUMMARY.md .planning/spikes/CONVENTIONS.md
```
</step>
...
`/gtd:spike` (run with no argument — its frontier mode analyzes the spike landscape and proposes integration and frontier spikes)

───────────────────────────────────────────────────────────────

**Also available:**
- `/gtd:plan-phase` — start planning the real implementation
- `/gtd:spike [idea]` — spike a specific new idea
...
- [ ] Summary presented
- [ ] Next-step options presented (including frontier spike exploration via `/gtd:spike`)
</success_criteria>
