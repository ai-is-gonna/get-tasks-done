<purpose>
Interactive configuration of third-party integrations for GTD — search API keys
(Brave / Firecrawl / Exa), code-review CLI routing (`review.models.<cli>`), and
agent-skill injection (`agent_skills.<agent-type>`). Writes to
`.planning/config.json` via `gtd-sdk`/`gtd-tools` so unrelated keys are
preserved, never clobbered.

This command is deliberately separate from `/gtd:settings` (workflow toggles)
and any `/gtd-settings-advanced` tuning surface. It exists because API keys and
...
- **`config-set` output is masked** for keys in the secret set
  ...
  `get-tasks-done/bin/lib/secrets.cjs`.
...
Store `$GTD_CONFIG_PATH`. Every subsequent read/write uses it.
</step>
...
```bash
BRAVE=$(gtd-sdk query config-get brave_search --default null)
FIRECRAWL=$(gtd-sdk query config-get firecrawl --default null)
EXA=$(gtd-sdk query config-get exa_search --default null)
SEARCH_GITIGNORED=$(gtd-sdk query config-get search_gitignored --default false)
```
...
```bash
gtd-sdk query config-set brave_search "<value>"     # masked in output
gtd-sdk query config-set firecrawl "<value>"        # masked in output
gtd-sdk query config-set exa_search "<value>"       # masked in output
gtd-sdk query config-set search_gitignored true|false
```

For "Clear", write `null`:

```bash
gtd-sdk query config-set brave_search null
```
</step>
...
```bash
gtd-sdk query config-set review.models.<cli> "<command string>"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "gtd-task-executor", description: "Skills injected when spawning executor agents" },
      { label: "gtd-planner", description: "Skills injected when spawning planner agents" },
      { label: "gtd-verifier", description: "Skills injected when spawning verifier agents" },
...
```bash
gtd-sdk query config-set agent_skills.<slug> "<skill-a,skill-b,skill-c>"
```
...
Quick commands:
- /gtd:settings — workflow toggles and model profile
- /gtd-set-profile <profile> — switch model profile
```
</step>
...
<success_criteria>
- [ ] Current config read from `$GTD_CONFIG_PATH`
