...
- gtd-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(gtd-sdk query agent-skills gtd-security-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(gtd-sdk query resolve-model gtd-security-auditor --raw)
SECURITY_CFG=$(gtd-sdk query config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
```

If `SECURITY_CFG` is `false`: exit with "Security enforcement disabled. Enable via /gtd:settings."

Display banner: `GTD > SECURE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /gtd:work-task-issue --phase {N} first."
...
```
Agent(
  prompt="Read ~/.claude/agents/gtd-security-auditor.md for instructions.\n\n" +
  ...
  subagent_type="gtd-security-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/get-tasks-done/templates/SECURITY.md`
...
```
GTD > PHASE {N} SECURITY BLOCKED
{K} threats open — phase advancement blocked until threats_open: 0
▶ Fix mitigations then re-run: /gtd:secure-phase {N}
...
```bash
gtd-sdk query commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
```
GTD > PHASE {N} THREAT-SECURE
threats_open: 0 — all threats have dispositions.
▶ /gtd:validate-phase {N}    validate test coverage
▶ /gtd:verify-work {N}       run UAT
```
