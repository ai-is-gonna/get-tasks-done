<purpose>
Lightweight codebase assessment. Spawns a single gtd-codebase-mapper agent for one focus area,
...
```bash
INIT=$(gtd-sdk query init.map-codebase 2>/dev/null || echo "{}")
...
Spawn a single `gtd-codebase-mapper` agent with the selected focus area:
...
Use `/gtd:map-codebase` for a comprehensive 4-area parallel scan.
```
