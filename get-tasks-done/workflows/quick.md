<purpose>
Execute small, ad-hoc tasks with GTD guarantees (atomic commits, STATE.md tracking). Quick mode spawns gtd-planner (quick mode) + gtd-task-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
- gtd-task-executor — Executes plan tasks, commits, creates SUMMARY.md
- gtd-verifier — Verifies phase completion, checks quality gates
- gtd-code-reviewer — Reviews source files for bugs, security issues, and code quality
</available_agent_types>
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► QUICK TASK (DISCUSS + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► QUICK TASK (DISCUSS + RESEARCH)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GTD ► QUICK TASK (RESEARCH + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
if ! command -v gtd-sdk &>/dev/null; then
  echo "⚠ gtd-sdk not found in PATH — /gtd:quick requires it."
  echo ""
  echo "Install the query-capable GTD SDK CLI:"
  echo "  npm install -g @ai-is-gonna/get-tasks-done"
  echo ""
  echo "Or update GTD to get the latest packages:"
  echo "  /gtd:update"
  exit 1
fi
```
...
```bash
INIT=$(gtd-sdk query init.quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(gtd-sdk query agent-skills gtd-planner)
AGENT_SKILLS_EXECUTOR=$(gtd-sdk query agent-skills gtd-task-executor)
AGENT_SKILLS_CHECKER=$(gtd-sdk query agent-skills gtd-plan-checker)
AGENT_SKILLS_VERIFIER=$(gtd-sdk query agent-skills gtd-verifier)
```

Parse JSON for: `planner_model`, `executor_model`, `checker_model`, `verifier_model`, `commit_docs`, `branch_name`, `quick_id`, `slug`, `date`, `timestamp`, `quick_dir`, `task_dir`, `roadmap_exists`, `planning_exists`.

```bash
USE_WORKTREES=$(gtd-sdk query config-get workflow.use_worktrees 2>/dev/null || echo "true")
```
...
if [ "${USE_WORKTREES}" != "false" ]; then
  COMMIT_DOCS=$(gtd-sdk query config-get commit_docs 2>/dev/null || echo "true")
  if [ "$COMMIT_DOCS" != "false" ]; then
    ...
    if git diff --cached --quiet -- "${QUICK_DIR}/${quick_id}-PLAN.md"; then
      ...
      SKIP_HOOKS=$(gtd-sdk query config-get workflow.worktree_skip_hooks 2>/dev/null || echo "false")
...
if [ "${USE_WORKTREES:-true}" != "false" ]; then
  QUICK_WORKTREE_MANIFEST=$(mktemp "${TMPDIR:-/tmp}/gtd-quick-worktree-XXXXXX.json")
  printf '{"worktrees":[]}\n' > "$QUICK_WORKTREE_MANIFEST"
  export QUICK_WORKTREE_MANIFEST
fi
```

Spawn gtd-task-executor with plan reference:
...
",
  subagent_type="gtd-task-executor",
...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   if command -v gtd-sdk >/dev/null 2>&1; then
     gtd-sdk query worktree.cleanup-wave --manifest "$QUICK_WORKTREE_MANIFEST" || exit 1
   else
     echo "WARN: gtd-sdk unavailable; using manifest-scoped shell fallback (#3384)." >&2
   ...
   WT_PATHS_FILE=$(mktemp "${TMPDIR:-/tmp}/gtd-worktree-paths-XXXXXX")
...
Note: For quick tasks producing multiple plans (rare), spawn executors in parallel waves per task orchestration flow patterns.
...
Agent(
  ...
  Depth: quick",
  subagent_type="gtd-code-reviewer",
  model="{executor_model}"
)
```
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `gtd-sdk query commit` command (or legacy `gtd-tools.cjs` commit) handles already-committed files gracefully.
...
gtd-sdk query commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
GTD > QUICK TASK COMPLETE (VALIDATED)
