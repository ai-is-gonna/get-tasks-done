...
Examples:
  `/gtd-edit-phase 5`       → phase = 5, force = false
  `/gtd-edit-phase 5 --force` → phase = 5, force = true
  `/gtd-edit-phase 12.1`    → phase = 12.1, force = false

If no argument provided:

```
ERROR: Phase number required
Usage: /gtd-edit-phase <phase-number> [--force]
Example: /gtd-edit-phase 5
Example: /gtd-edit-phase 5 --force
```
...
```bash
INIT=$(gtd-sdk query init.phase-op "${target}")
...
```bash
PHASE_DATA=$(gtd-sdk query roadmap get-phase "${target}")
```
...
Available phases can be seen with /gtd:progress.
```
...
```bash
ALL_PHASES=$(gtd-sdk query roadmap analyze)
```
...
- `/gtd:progress` — view updated roadmap
- `/gtd:plan-phase {target}` — re-plan this phase (if needed)
- `/gtd:discuss-phase {target}` — discuss implementation approach
