...
- gtd-doc-writer — Writes and updates project documentation files
- gtd-doc-verifier — Verifies factual claims in docs against the live codebase
</available_agent_types>
...
```bash
INIT=$(gtd-sdk query docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(gtd-sdk query agent-skills gtd-doc-writer)
```
...
- `existing_docs` — array of `{path, has_gtd_marker}` objects for existing Markdown files
...
- These will be passed to gtd-doc-verifier in the verify_docs step for accuracy checking
- If inaccuracies are found, they will be dispatched to gtd-doc-writer in `fix` mode for surgical corrections
...
4. For each gap the user selects:
   ...
   - The gtd-doc-writer will receive a `doc_assignment` with `type: "custom"` and a description of what to document, using the project's source files as content discovery targets
...
3. If no docs in the queue have `has_gtd_marker: false` in the `existing_docs` array: skip to detect_runtime_capabilities.

**For each queued doc where `has_gtd_marker` is false (hand-written doc detected):**

Present the following choice using `AskUserQuestion` if available, or inline prompt otherwise:

```
{filename} appears to be hand-written (no GTD marker found).
...
How should this file be handled?
  ...
  [3] regenerate  -- Overwrite with a fresh GTD-generated doc.
```
...
Spawn 3 parallel gtd-doc-writer agents for Wave 1 docs: README, ARCHITECTURE, CONFIGURATION.
...
Spawn a `gtd-doc-writer` agent with `run_in_background=true`:
...
**Fallback when Task tool is unavailable:** Generate per-package READMEs sequentially inline after the `sequential_generation` step. For each package directory with a `package.json`, construct the equivalent `doc_assignment` block and generate the README following gtd-doc-writer instructions.
...
Read `agents/gtd-doc-writer.md` instructions once before beginning. Follow the create_mode or update_mode instructions from that agent for each doc, using the same doc_assignment fields as the parallel path.
...
1. **README** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase (Read, Grep, Glob, Bash) following gtd-doc-writer create_mode / update_mode instructions
...
2. **ARCHITECTURE** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase following gtd-doc-writer instructions
...
- Follow gtd-doc-writer instructions for per_package scope
...
1. Spawn the `gtd-doc-verifier` agent (or invoke sequentially if Task tool is unavailable) with a `<verify_assignment>` block:
   ```xml
   <verify_assignment>
...
1. Spawn the `gtd-doc-verifier` agent with the same `<verify_assignment>` block as above.
...
Non-canonical docs with failures ARE eligible for the fix_loop. When a non-canonical doc has `claims_failed > 0`, dispatch it to gtd-doc-writer in `fix` mode with the failures array — the writer's fix mode does surgical corrections on specific lines regardless of doc type (no template needed). The writer MUST NOT restructure, rephrase, or reformat any content beyond the failing claims.
...
1. For each doc with `claims_failed > 0` in the latest verification_results:
   ...
   b. Spawn `gtd-doc-writer` agent (or invoke sequentially) with a fix assignment:
      ```xml
      <doc_assignment>
...
Invoke the gtd-doc-verifier agent in read-only mode for each file in `existing_docs` from the init JSON:

1. For each doc in `existing_docs`:
   a. Spawn `gtd-doc-verifier` (or invoke sequentially if Task tool is unavailable) with:
      ```xml
      <verify_assignment>
      doc_path: {doc.path}
      project_root: {project_root from init JSON}
...
Display note:
```
To fix failures automatically: /gtd:docs-update (runs generation + fix loop)
To regenerate all docs from scratch: /gtd:docs-update --force
```
...
3. Re-run /gtd:docs-update to regenerate clean docs
```
...
```bash
gtd-sdk query commit "docs: generate project documentation" \
...
```
Run `/gtd:docs-update --verify-only` to fact-check generated docs against the codebase.
```
...
- [ ] Generated docs contain zero GTD methodology content
...
- [ ] Hand-written docs (no GTD marker) prompted for preserve/supplement/regenerate before dispatch (unless --force)
...
- [ ] --verify-only invokes gtd-doc-verifier for full fact-checking (not just VERIFY marker count)
</success_criteria>
