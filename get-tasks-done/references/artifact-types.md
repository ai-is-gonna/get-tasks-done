...
This reference documents all artifact types in the GTD planning taxonomy. Each type has a defined
...
- **Consumed by**: `plan-phase`, `discuss-phase`, `task orchestration flow`, `progress`, `state` commands
...
- **Consumed by**: `plan-phase` (reads decisions), `task orchestration flow` (reads code_context and canonical_refs)
...
- **Consumed by**: `task orchestration flow` executor; task commits reference plan IDs
...
- **Location**: `~/.claude/get-tasks-done/USER-PROFILE.md`
...
### Spike README.md / MANIFEST.md (per-spike, via /gtd:spike)
- **Shape**: YAML frontmatter (spike, name, validates, verdict, related, tags) + run instructions + results
- **Lifecycle**: Created by `/gtd:spike` → Verified → Wrapped up by `/gtd-spike-wrap-up`
- **Location**: `.planning/spikes/NNN-name/README.md`, `.planning/spikes/MANIFEST.md`
- **Consumed by**: `/gtd-spike-wrap-up` for curation; `pause-work` for spike context handoff
