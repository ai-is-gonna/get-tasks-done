...
Display:
```
GTD > BLOCKED: {N} blockers must be resolved before {operation} can proceed.
```
...
Proceed silently or display `GTD > No conflicts detected.` Either is acceptable; workflows choose based on verbosity context.
