...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever GTD itself can save through `model_profile` tuning. This is a Claude Code harness concern, not a GTD concern: GTD does **not** manage MCP enablement. The toggle lives in `.claude/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
Before starting a long phase (especially `/gtd:work-task-issue --phase`, `/gtd:plan-phase`, or anything that fans out across many subagents), run this audit:
