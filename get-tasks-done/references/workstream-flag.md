...
The `--ws <name>` flag scopes GTD operations to a specific workstream, enabling
parallel milestone work by multiple Claude Code instances on the same codebase.

## Resolution Priority

1. `--ws <name>` flag (explicit, highest priority)
2. `GTD_WORKSTREAM` environment variable (per-instance)
...
GTD now prefers a session-scoped pointer keyed by runtime/session identity
(`GTD_SESSION_KEY`, `CODEX_THREAD_ID`, `CLAUDE_CODE_SSE_PORT`, terminal session IDs,
...
When GTD resolves the session-scoped pointer in step 3 above, it uses this order:

1. Explicit runtime/session env vars such as `GTD_SESSION_KEY`, `CODEX_THREAD_ID`,
...
If none of those produce a stable identity, GTD does not keep probing. It falls
...
non-interactive, GTD intentionally skips shelling out to `tty` because that path
...
- If that was the last pointer for the repo, GTD also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, GTD
...
GTD does not currently run a background garbage collector for historical temp
...
All workflow routing commands include `${GTD_WS}` which:
...
`new-milestone → discuss-phase → plan-phase → task orchestration flow → transition`
...
```bash
# All gtd-sdk query commands accept --ws
gtd-sdk query state.json --ws feature-a
gtd-sdk query find-phase 3 --ws feature-b

# Session-local switching without --ws on every command
GTD_SESSION_KEY=my-terminal-a gtd-sdk query workstream.set feature-a
GTD_SESSION_KEY=my-terminal-a gtd-sdk query state.json
GTD_SESSION_KEY=my-terminal-b gtd-sdk query workstream.set feature-b
GTD_SESSION_KEY=my-terminal-b gtd-sdk query state.json

# Workstream CRUD
gtd-sdk query workstream.create <name>
gtd-sdk query workstream.list
gtd-sdk query workstream.status <name>
gtd-sdk query workstream.complete <name>
```
