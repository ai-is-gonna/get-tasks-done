# Task Issue MVP+TDD Gate (Runtime Enforcement)

> Loaded by the task issue workflow and `gtd-task-executor` agent only when **both** `MVP_MODE=true` AND `TDD_MODE=true` for the phase. Defines the runtime gate that blocks behavior-adding tasks until a failing-test commit exists.
...
2. Emit a structured halt report:
   ...
   3. Re-run `/gtd:work-task-issue --phase {phase}`
   ```
...
The existing end-of-phase TDD review (in `workflows/work-task-issue.md`'s `tdd_review_checkpoint` step) is normally **advisory** — it surfaces gate violations but does not block phase completion.
...
- The user is shown the same review table, but the verdict line reads:
  > "Phase blocked: {N} TDD plan(s) violate the RED→GREEN gate sequence under MVP+TDD. Resolve and re-run `/gtd:work-task-issue --phase {phase}`."

There is no phase-level override in this flow. Resolve the affected task issue or update the plan before finalization.
