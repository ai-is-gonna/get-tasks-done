...
| `git.phase_branch_template` | `"gtd/phase-{phase}-{slug}"` | Branch template for phase strategy |
| `git.milestone_branch_template` | `"gtd/{milestone}-{slug}"` | Branch template for milestone strategy |
...
| `manager.flags.discuss` | `""` | Flags passed to `/gtd:discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
**Using `gtd-sdk query` (preferred):**

```bash
# Commit with automatic commit_docs + gitignore checks:
gtd-sdk query commit "docs: update state" --files .planning/STATE.md
...
INIT=$(gtd-sdk query init.plan-phase "1")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
...
**Note:** Most GTD operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
|----------|---------------------|--------------|-------------|
| `none` | Never | N/A | N/A |
| `phase` | At `task orchestration flow` start | Single phase | User merges after phase |
| `milestone` | At first `task orchestration flow` of milestone | Entire milestone | At `complete-milestone` |

**When `git.branching_strategy: "none"` (default):**
- All work commits to current branch
- Standard GTD behavior

**When `git.branching_strategy: "phase"`:**
- `task orchestration flow` creates/switches to a branch before execution
- Branch name from `phase_branch_template` (e.g., `gtd/phase-03-authentication`)
- All plan commits go to that branch
- User merges branches manually after phase completion
- `complete-milestone` offers to merge all phase branches

**When `git.branching_strategy: "milestone"`:**
- First `task orchestration flow` of milestone creates the milestone branch
- Branch name from `milestone_branch_template` (e.g., `gtd/v1.0-mvp`)
...
Use `init plan-phase` which returns all config as JSON:
```bash
INIT=$(gtd-sdk query init.plan-phase "1")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
...
| `review.models.<cli>` | string\|null | `null` | Any model ID string | Per-CLI model override for /gtd:review (e.g., `review.models.gemini`). Falls back to CLI default when null. |
...
| `workflow.ai_integration_phase` | boolean | `true` | `true`, `false` | Run /gtd:ai-integration-phase before planning AI system phases |
...
| `workflow.code_review` | boolean | `true` | `true`, `false` | Enable `/gtd:code-review` and `/gtd:code-review --fix` |
| `workflow.code_review_depth` | string | `"standard"` | `"quick"`, `"standard"`, `"deep"` | Default depth for `/gtd:code-review` analysis |
| `workflow._auto_chain_active` | boolean | `false` | `true`, `false` | Internal: tracks whether autonomous chaining is active |
| `workflow.security_enforcement` | boolean | `true` | `true`, `false` | Enable threat-model-anchored security verification via `/gtd:secure-phase`. When `false`, security checks are skipped entirely |
...
| `git.phase_branch_template` | string | `"gtd/phase-{phase}-{slug}"` | Template with `{phase}`, `{slug}` | Branch naming template for `phase` strategy |
| `git.milestone_branch_template` | string | `"gtd/{milestone}-{slug}"` | Template with `{milestone}`, `{slug}` | Branch naming template for `milestone` strategy |
...
| `features.global_learnings` | boolean | `false` | `true`, `false` | Enable injection of global learnings from `~/.gtd/learnings/` into agent prompts |
...
Set via `intel.*` namespace (e.g., `"intel": { "enabled": true }`). Controls the queryable codebase intelligence system consumed by `/gtd:map-codebase --query`.

| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `intel.enabled` | boolean | `false` | `true`, `false` | Enable queryable codebase intelligence system. When `true`, `/gtd:map-codebase --query` builds and queries a JSON index in `.planning/intel/`. |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/gtd:discuss-phase` from manager (e.g., `"--auto --analyze"`) |
...
5. **Search API keys and flags** -- `brave_search`, `firecrawl`, and `exa_search` are auto-set to `true` during project creation if the corresponding API key is detected (environment variable or `~/.gtd/<name>_api_key` file). Setting them to `true` without the API key has no effect.
...
8. **`sub_repos` auto-sync** -- On every config load, GTD scans for child directories with `.git` and updates the `sub_repos` array if the filesystem has changed. Legacy `multiRepo: true` is automatically migrated to a detected `sub_repos` array.
