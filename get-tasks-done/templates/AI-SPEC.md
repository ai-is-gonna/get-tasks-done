...
> AI design contract generated by `/gtd:ai-integration-phase`. Consumed by `gtd-planner` and `gtd-eval-auditor`.
...
> Researched by `gtd-domain-researcher`. Grounds the evaluation strategy in domain expert knowledge.
...
> Fetched from official docs by `gtd-ai-researcher`. Distilled for this specific use case.
...
> Written by `gtd-ai-researcher`. Cross-cutting patterns every developer building AI systems needs — independent of framework choice.
