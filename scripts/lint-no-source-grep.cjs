 ...
 *
 * ALLOWED:
 *   - require('../get-tasks-done/bin/lib/foo.cjs')  -- runs the module, not text inspection
...
// Requires a source-dir indicator ('bin', 'lib', 'get-tasks-done') to avoid
// flagging temp files like path.join(tmpDir, 'example.cjs').
//   const CONFIG_PATH = path.join(__dirname, '..', 'get-tasks-done', 'bin', 'lib', 'config-schema.cjs');
const CJS_PATH_CONST_RE = /(?:const|let|var)\s+(\w+)\s*=\s*path\.join\s*\([^)]*(?:'bin'|"bin"|'lib'|"lib"|'get-tasks-done'|"get-tasks-done")[^)]*['"][^'"]*\.cjs['"]/gm;
...
const READ_WITH_INLINE_CJS_RE = /readFileSync\s*\([^,)]*path\.join\s*\([^)]*(?:'bin'|"bin"|'lib'|"lib"|'get-tasks-done'|"get-tasks-done")[^)]*['"][^'"]*\.cjs['"]/;
...
function check(filepath) {
  ...
  if (READ_WITH_INLINE_CJS_RE.test(content)) {
    violations.push({
      ...
      fix: 'Replace with runGtdTools() behavioral test, or add // allow-test-rule: <reason>',
    });
  }
