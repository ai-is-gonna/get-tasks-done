...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `~/.claude/skills/.gtd-surface.json`
(sibling to `.gtd-profile`) and re-stages the active commands/gtd directory in place.
...
| `reset` | Delete `.gtd-surface.json`, return to install-time profile |
| *(none)* | Treat as `list` |
...
`get-tasks-done/bin/lib/surface.cjs`. Display:

```
Enabled (N skills, ~T tokens):
  core_loop:   new-project  discuss-phase  plan-phase  export-phase-issues  work-task-issue  orchestrate-tasks  help  update
  audit_review: …
  …
...
```
Base profile:   standard  (from .gtd-surface.json)
Install profile: standard  (from .gtd-profile)
```
...
Valid cluster names: `core_loop`, `issue_task_loop`, `audit_review`, `milestone`,
`research_ideate`, `workspace_state`, `docs`, `ui`, `ai_eval`, `ns_meta`, `utility`.
...
1. Check if `.gtd-surface.json` exists.
2. Delete it.
...
COMMANDS_DIR=~/.claude/commands/gtd
...
- Unknown profile name → list known profiles (`core`, `issue-tasks`, `standard`, `full`), exit.
- The issue task loop can be surfaced with `profile issue-tasks` or by enabling `issue_task_loop`.
- Missing `surface.cjs` → prompt: "Run `npm i -g get-tasks-done` to reinstall GTD."

<execution_context>
Surface state file: `~/.claude/skills/.gtd-surface.json`
Install profile marker: `~/.claude/skills/.gtd-profile`
Engine module: `~/.claude/get-tasks-done/bin/lib/surface.cjs`
Cluster definitions: `~/.claude/get-tasks-done/bin/lib/clusters.cjs`
</execution_context>
