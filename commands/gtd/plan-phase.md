...
**Research-only mode (`--research-phase <N>`):** Spawn `gtd-phase-researcher` for phase `N`, write `RESEARCH.md`, then exit before the planner runs. Useful for cross-phase research, doc review before committing to a planning approach, and correction-without-replanning loops where iterating on research alone is dramatically cheaper than re-spawning the planner. Replaces the deleted `/gtd-research-phase` command (#3042).
...
<execution_context>
@~/.claude/get-tasks-done/workflows/plan-phase.md
...
- `--reviews` — Replan incorporating cross-AI review feedback from REVIEWS.md (produced by `/gtd:review`)
