...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gtd-sdk query`. Use `node $HOME/.claude/get-tasks-done/bin/gtd-tools.cjs graphify …` as documented in this command. Other tooling may still use `gtd-sdk query` where a handler exists.
...
**DO NOT use the gtd-tools config get-value command** -- it hard-exits on missing keys.
...
Knowledge graph is disabled. To activate:
  ...
  node $HOME/.claude/get-tasks-done/bin/gtd-tools.cjs config-set graphify.enabled true

Then run /gtd:graphify build to create the initial graph.
```
...
```bash
node $HOME/.claude/get-tasks-done/bin/gtd-tools.cjs graphify query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If the output contains `"error"` field, display the error message and **STOP**
- If no nodes found, display: `No graph matches for '<term>'. Try /gtd:graphify build to create or rebuild the graph.`
...
```bash
node $HOME/.claude/get-tasks-done/bin/gtd-tools.cjs graphify status
```
...
```bash
node $HOME/.claude/get-tasks-done/bin/gtd-tools.cjs graphify diff
```
...
```bash
node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" graphify build
```
...
graphify update . \
  ...
  && node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" graphify build snapshot \
  && node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" graphify status
```
...
**MVP-mode rendering.** When a phase has `**Mode:** mvp` in ROADMAP.md (resolved via `gtd-sdk query roadmap.get-phase --pick mode`), render its graph node with two distinct visual signals:
...
5. DO NOT use `gtd-tools config get-value` for the config gate -- it exits on missing keys.
