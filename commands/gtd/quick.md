...
- Spawns gtd-planner (quick mode) + gtd-task-executor(s)
...
<execution_context>
@~/.claude/get-tasks-done/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  gtd-sdk query frontmatter.get .planning/quick/{dir}/SUMMARY.md status
  ```
...
─────────────────────────────────────
Resume with: /gtd:quick resume {slug}
```
