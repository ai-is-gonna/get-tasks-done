...
allowed-tools:
  ...
  - Grep
  - AskUserQuestion
requires: [discuss-phase, export-phase-issues, phase, plan-phase, work-task-issue]
---

<objective>
Clarify phase requirements through structured Socratic questioning with quantitative ambiguity scoring.

**Position in workflow:** `spec-phase → discuss-phase → plan-phase → export task issues → work task issues → verify`
...
<execution_context>
@~/.claude/get-tasks-done/workflows/spec-phase.md
@~/.claude/get-tasks-done/templates/spec.md
</execution_context>
...
- User knows they can now run /gtd:discuss-phase which will load SPEC.md automatically
</success_criteria>
