...
4. Delegates to `/gtd plan-phase <N>` which auto-detects MVP mode via the roadmap field

Phase 1 of the vertical-mvp-slice PRD shipped the planner-side machinery; this command is the user entry point for it.
</objective>

<execution_context>
@~/.claude/get-tasks-done/workflows/mvp-phase.md
...
The phase must already exist in ROADMAP.md (created via `/gtd new-project`, `/gtd add-phase`, or `/gtd insert-phase`). This command does not create new phases — it converts an existing phase to MVP mode.
</context>

<process>
Execute the mvp-phase workflow from @~/.claude/get-tasks-done/workflows/mvp-phase.md end-to-end.
