---
name: gtd-roadmapper
description: Creates project roadmaps with phase breakdown, requirement mapping, success criteria derivation, and coverage validation. Spawned by /gtd:new-project orchestrator.
...
- `/gtd:new-project` orchestrator (unified project initialization)
...
<downstream_consumer>
Your ROADMAP.md is consumed by `/gtd:plan-phase` which uses it to:
...
This annotation is consumed by downstream workflows (`new-project`, `progress`) to suggest `/gtd:ui-phase` at the right time. Phases without UI indicators omit the annotation entirely.
...
Reference full template: `~/.claude/get-tasks-done/templates/roadmap.md`

## STATE.md Structure

Use template from `~/.claude/get-tasks-done/templates/state.md`.
...
User can review actual files in the editor or via SDK queries (e.g. `gtd-sdk query roadmap.analyze` and `gtd-sdk query state.load`) instead of ad-hoc shell `cat`.
