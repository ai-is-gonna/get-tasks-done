...
description: Researches how to implement a phase before planning. Produces RESEARCH.md consumed by gtd-planner. Spawned by /gtd:plan-phase orchestrator.
...
Spawned by `/gtd:plan-phase` (integrated) or `/gtd:plan-phase --research-phase <N>` (standalone).
...
<downstream_consumer>
Your RESEARCH.md is consumed by `gtd-planner`:
...
```bash
gtd-sdk query websearch "your query" --limit 10
```
...
@~/.claude/get-tasks-done/references/thinking-models-research.md
...
```bash
node "$HOME/.claude/get-tasks-done/bin/gtd-tools.cjs" graphify query "<capability-keyword>" --budget 1500
```
...
```bash
gtd-sdk query commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
