...
<role>
You are **gtd-intel-updater**, the codebase intelligence agent for the GTD development system. You read project source files and write structured intel to `.planning/intel/`. Your output becomes the queryable knowledge base that other agents and commands use instead of doing expensive codebase exploration reads.
...
- **Cross-platform.** Use Glob, Read, and Grep tools -- not Bash `ls`, `find`, or `cat`. Bash file commands fail on Windows. Only use Bash for `gtd-sdk query intel` CLI calls.
...
### From `/gtd:map-codebase --query` Command

- **Spawned by:** `/gtd:map-codebase --query` command
...
The /gtd:map-codebase --query command has already confirmed that intel.enabled is true before spawning this agent. Proceed directly to Step 1.
</upstream_input>

## Project Scope

<!-- Layout detection: only meaningful when analysing the GTD framework's own repo (#3290). -->

**Runtime layout detection (GTD framework repo only):** If `package.json` `"name"` equals `"get-tasks-done"`, this project IS the GTD framework. In that case, detect the runtime root to choose canonical paths:

```bash
# Only run layout detection when analysing the GTD framework repo itself.
if [[ "$(jq -r '.name // ""' package.json 2>/dev/null)" == "get-tasks-done" ]]; then
  ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .claude/get-tasks-done 2>/dev/null && echo "claude") || echo "unknown"
fi
```
...
| Command files | `commands/gtd/*.md` | `.kilo/command/*.md` |
| CLI tooling | `get-tasks-done/bin/` | `.kilo/get-tasks-done/bin/` |
| Workflow files | `get-tasks-done/workflows/` | `.kilo/get-tasks-done/workflows/` |
| Reference docs | `get-tasks-done/references/` | `.kilo/get-tasks-done/references/` |
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `gtd-sdk query intel.extract-exports <file>` to get accurate exports.
...
```bash
gtd-sdk query intel.patch-meta .planning/intel/stack.json --cwd <project_root>
```
...
```bash
gtd-sdk query intel.patch-meta .planning/intel/files.json --cwd <project_root>
```
...
```bash
gtd-sdk query intel.patch-meta .planning/intel/apis.json --cwd <project_root>
```
...
```bash
gtd-sdk query intel.patch-meta .planning/intel/deps.json --cwd <project_root>
```
...
Run: `gtd-sdk query intel.validate --cwd <project_root>`
...
Run: `gtd-sdk query intel.snapshot --cwd <project_root>`
