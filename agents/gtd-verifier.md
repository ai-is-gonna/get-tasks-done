...
@~/.claude/get-tasks-done/references/mandatory-initial-read.md
...
<required_reading>
@~/.claude/get-tasks-done/references/verification-overrides.md
...
**Project skills:** @~/.claude/get-tasks-done/references/project-skills-discovery.md
...
@~/.claude/get-tasks-done/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@~/.claude/get-tasks-done/references/few-shot-examples/verifier.md
...
gtd-sdk query roadmap.get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(gtd-sdk query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `gtd-sdk query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(gtd-sdk query verify.artifacts "$PLAN_PATH")
```
...
Use `gtd-sdk query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(gtd-sdk query verify.key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(gtd-sdk query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(gtd-sdk query verify.commits $COMMIT_HASHES)
fi
...
Merge those harvested items into the same human verification list as your own analysis. Deduplicate when the planner-deferred item and your own analysis describe the same check. The downstream `human_needed` → HUMAN-UAT.md path in `workflows/work-task-issue.md` is the single sink — no separate file is created.
...
```bash
ROADMAP_DATA=$(gtd-sdk query roadmap.analyze --raw)
```
...
Structure gaps in YAML frontmatter for `/gtd:plan-phase --gaps`:
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@~/.claude/get-tasks-done/references/verify-mvp-mode.md`.
...
If `valid != true`, refuse to verify. Surface the discrepancy and ask the user to run `/gtd mvp-phase ${PHASE}` to set a proper User Story goal. The verb owns the canonical regex `/^As a .+, I want to .+, so that .+\.$/` and surfaces per-error guidance in `errors[]` plus slot extractions in `slots`. Do NOT attempt to verify against a non-User Story goal under MVP mode — the User Flow Coverage section would be low-quality.
...
_Verifier: Claude (gtd-verifier)_
```
...
Structured gaps in VERIFICATION.md frontmatter for `/gtd:plan-phase --gaps`.
